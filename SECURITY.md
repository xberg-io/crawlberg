...
Crawlberg fetches and parses content from the open web, so it processes **hostile input by default**. In scope: server-side request forgery and access to link-local or internal addresses, credential or header leakage across redirects, resource exhaustion from hostile pages, parser crashes and panics, and failure to honour crawl-scope restrictions.
