...
<!-- alef:hash:4ded10935a13e13dfd7428fe0696e39ef8ae8b89002923779b7cbb186d73d230 -->
...
```c
CBERGAlefHandle cberg_interact(CBERGAlefHandle engine, const char* url, CBERGAlefHandle* actions);
```
...
| `engine` | `CBERGAlefHandle` | Yes | The crawl engine handle |
| `url` | `const char*` | Yes | The URL to fetch |
| `actions` | `CBERGAlefHandle*` | Yes | The actions |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `CBERGAlefHandle*` | `NULL` | Per-URL crawl results, in the order seed URLs were submitted. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `CBERGAlefHandle*` | `NULL` | Per-URL scrape results, in the order URLs were submitted. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `CBERGAlefHandle` | `CBERG_CBERG_AUTO` | When to use the headless browser fallback. |
| `backend` | `CBERGAlefHandle` | `CBERG_CBERG_CHROMIUMOXIDE` | Browser backend used to render JavaScript-heavy pages. |
...
| `wait` | `CBERGAlefHandle` | `CBERG_CBERG_NETWORK_IDLE` | Wait strategy after browser navigation. |
...
| `network_events` | `CBERGAlefHandle*` | `NULL` | Network events captured during page navigation (only populated when `BrowserConfig.capture_network_events` is true). |
| `cookies` | `CBERGAlefHandle*` | `NULL` | All non-expired cookies present in the browser's cookie jar after navigation completes (includes both prior cookies and server Set-Cookie). |
...
| `references` | `CBERGAlefHandle*` | `NULL` | Numbered reference list: (index, url, text). |
...
| `crawl_strategy` | `CBERGAlefHandle` | `CBERG_CBERG_BFS` | Traversal order. Defaults to breadth-first. A frontier or strategy set explicitly on `CrawlEngineBuilder` takes precedence over this field. |
...
| `asset_types` | `CBERGAlefHandle*` | `NULL` | Filter for asset categories to download. |
...
| `links` | `CBERGAlefHandle*` | `NULL` | Links found on the page. |
| `images` | `CBERGAlefHandle*` | `NULL` | Images found on the page. |
| `feeds` | `CBERGAlefHandle*` | `NULL` | Feed links found on the page. |
| `json_ld` | `CBERGAlefHandle*` | `NULL` | JSON-LD entries found on the page. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pages` | `CBERGAlefHandle*` | `NULL` | The list of crawled pages. |
...
| `cookies` | `CBERGAlefHandle*` | `NULL` | Cookies collected during the crawl. |
...
| `asset_category` | `CBERGAlefHandle` | `CBERG_CBERG_IMAGE` | The category of the asset. |
...
| `feed_type` | `CBERGAlefHandle` | `CBERG_CBERG_RSS` | The type of feed. |
...
| `source` | `CBERGAlefHandle` | `CBERG_CBERG_IMG` | The source of the image reference. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `action_results` | `CBERGAlefHandle*` | `NULL` | Results from each executed action. |
...
| `link_type` | `CBERGAlefHandle` | `CBERG_CBERG_INTERNAL` | The classification of the link. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `urls` | `CBERGAlefHandle*` | `NULL` | The list of discovered URLs. |
...
| `hreflangs` | `CBERGAlefHandle*` | `NULL` | Hreflang alternate links. |
| `favicons` | `CBERGAlefHandle*` | `NULL` | Favicon and icon links. |
| `headings` | `CBERGAlefHandle*` | `NULL` | Heading elements (h1-h6). |
...
| `assets` | `CBERGAlefHandle*` | `NULL` | Downloaded assets from the page. |
