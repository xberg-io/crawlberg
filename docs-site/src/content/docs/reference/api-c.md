...
<!-- alef:hash:04ed06d4a2ba87a6dc75a5e8c4db7507ce3cf13b40c30ba463d19783728e6587 -->
...
| `path_patterns_match_url` | `int32_t` | `false` | Whether `include_paths`/`exclude_paths` match against the full URL, `scheme://host[:port]/path?query`, so a pattern can scope by host. Defaults to `false`. When `true` it takes precedence over `Self.path_patterns_match_query`: the query is part of the full URL whatever that flag says. The matched text never contains a `user:password@`, a fragment or a default port, and the host is in punycode (`bücher.de` is matched as `xn--bcher-kva.de`). |
