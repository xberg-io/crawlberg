...
<!-- alef:hash:0e36c26364272f5b08cdbaeb98696b28d84a556e8a26269fdb6b7b88a0b50bb1 -->
...
| `noindex_detected` | `int32_t` | — | Whether the page asked not to be indexed, by its robots meta tag or `X-Robots-Tag` header. The page is still crawled and its links still followed. |
| `nofollow_detected` | `int32_t` | — | Whether the page asked that its links not be followed, by its robots meta tag or `X-Robots-Tag` header. When the crawl respects robots, its links are not followed. |
...
| `x_robots_tag` | `const char*` | `NULL` | The X-Robots-Tag header values, joined with `, ` when the response sent more than one. |
| `is_pdf` | `int32_t` | — | Whether the content is a PDF. |
