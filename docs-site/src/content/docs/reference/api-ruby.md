...
<!-- alef:hash:7bb9956fe02bb2eb4199af806a65e7753b810782dc689c33d1bab2078460ce1d -->
...
| `path_patterns_match_url` | `Boolean` | `false` | Whether `include_paths`/`exclude_paths` match against the full URL, `scheme://host[:port]/path?query`, so a pattern can scope by host. Defaults to `false`. When `true` it takes precedence over `Self.path_patterns_match_query`: the query is part of the full URL whatever that flag says. The matched text never contains a `user:password@`, a fragment or a default port, and the host is in punycode (`bücher.de` is matched as `xn--bcher-kva.de`). |
