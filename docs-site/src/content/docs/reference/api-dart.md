...
<!-- alef:hash:29fcff1327e1cda0a96ac034d4eacb1a57354a3089262f94226b24cd8c99a0cd -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Dart API Reference <span class="version-badge">v1.8.0</span>
...
```dart
Future<CitationResult> generateCitations(String markdown)
```
...
```dart
Future<ScrapeResult> scrape(CrawlEngineHandle engine, String url)
```
...
```dart
Future<CrawlResult> crawl(CrawlEngineHandle engine, String url)
```
...
```dart
Future<MapResult> mapUrls(CrawlEngineHandle engine, String url)
```
...
```dart
Future<InteractionResult> interact(CrawlEngineHandle engine, String url, List<PageAction> actions)
```
...
```dart
Future<BatchScrapeResults> batchScrape(CrawlEngineHandle engine, List<String> urls)
```
...
```dart
Future<BatchCrawlResults> batchCrawl(CrawlEngineHandle engine, List<String> urls)
```
...
|-------|------|---------|-------------|
| `actionIndex` | `PlatformInt64` | — | Zero-based index of the action in the sequence. |
...
| `totalCount` | `PlatformInt64` | — | Total number of seed URLs in the batch (equal to `results.len()`). |
| `completedCount` | `PlatformInt64` | — | Number of seed URLs whose crawl succeeded (`error` is `null`). |
| `failedCount` | `PlatformInt64` | — | Number of seed URLs whose crawl failed (`error` is `Some`). |
...
| `totalCount` | `PlatformInt64` | — | Total number of URLs in the batch (equal to `results.len()`). |
| `completedCount` | `PlatformInt64` | — | Number of URLs whose scrape succeeded (`error` is `null`). |
| `failedCount` | `PlatformInt64` | — | Number of URLs whose scrape failed (`error` is `Some`). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `index` | `PlatformInt64` | — | 1-based reference number as it appears in the source text. |
...
| `retryCount` | `PlatformInt64` | `0` | Number of retry attempts for failed requests. Bounded by `MAX_RETRY_COUNT`. |
| `retryCodes` | `List<int>` | `[]` | HTTP status codes that should trigger a retry. When empty, every rate limit, server error, bad gateway and timeout is retried. When set, only a failure whose status is listed is retried, so a timeout without a response is not. |
| `retryInitialDelayMs` | `int` | `100` | Initial delay, in milliseconds, before the first retry. Doubled on each subsequent attempt (capped at `retry_max_delay_ms`). Defaults to 100ms. |
| `retryMaxDelayMs` | `int` | `60000` | Upper bound, in milliseconds, on the exponential retry backoff. Defaults to 60s. |
...
| `bodySize` | `PlatformInt64` | — | The size of the response body in bytes. |
...
| `depth` | `PlatformInt64` | — | The depth of this page from the start URL. |
...
| `redirectCount` | `PlatformInt64` | — | Redirect hops taken to reach `final_url` from `url`. |
...
| `redirectCount` | `PlatformInt64` | — | The number of redirects followed. |
...
| `size` | `PlatformInt64` | — | The size of the asset in bytes. |
...
| `size` | `PlatformInt64` | — | Size of the document in bytes. |
...
| `chunksProcessed` | `PlatformInt64` | — | Number of content chunks sent to the LLM. |
...
| `tables` | `List<String>` | `[]` | Extracted tables with structured cell data. |
...
| `wordCount` | `PlatformInt64?` | `null` | Computed word count of the page body text. |
...
| `Complete` | The crawl has completed. — Fields: `pagesCrawled`: `PlatformInt64` |
