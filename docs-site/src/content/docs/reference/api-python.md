...
<!-- alef:hash:117837c9dfaef518f8336f084714054ef17867d693abde0f24b978330477dcbb -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Python API Reference <span class="version-badge">v1.8.0</span>
...
| `tracking_params` | `list\[str\]` | `["utm_*", "fbclid", "gclid", "ref"]` | Query parameter name patterns to strip when `strip_tracking_params` is `True`. A pattern ending in `*` matches by prefix (`utm_*` matches `utm_source`, `utm_campaign`, ...); any other pattern matches the parameter name exactly. Defaults to `["utm_*", "fbclid", "gclid", "ref"]`, applied only once `strip_tracking_params` is enabled. |
...
| `retry_count` | `int` | `0` | Number of retry attempts for failed requests. Bounded by `MAX_RETRY_COUNT`. |
| `retry_codes` | `list\[int\]` | `[]` | HTTP status codes that should trigger a retry. When empty, every rate limit, server error, bad gateway and timeout is retried. When set, only a failure whose status is listed is retried, so a timeout without a response is not. |
| `retry_initial_delay_ms` | `int` | `100` | Initial delay, in milliseconds, before the first retry. Doubled on each subsequent attempt (capped at `retry_max_delay_ms`). Defaults to 100ms. |
| `retry_max_delay_ms` | `int` | `60000` | Upper bound, in milliseconds, on the exponential retry backoff. Defaults to 60s. |
| `rate_limit_jitter_ratio` | `float` | `0` | Fraction of the per-domain rate-limit delay to randomly jitter by, in `[0.0, 1.0]`. `0.0` (the default) applies no jitter and preserves the previous fixed-interval behaviour; `0.1` jitters the delay by up to ±10%. |
...
| `final_url` | `str` | — | The URL this page's content was actually fetched from, after following any HTTP, `Refresh` header, or `<meta http-equiv="refresh">` redirect `url` pointed at. Equal to `url` when the fetch did not redirect. |
| `redirect_count` | `int` | — | Redirect hops taken to reach `final_url` from `url`. |
...
| `content_path` | `str \| None` | `None` | Filesystem path the document was streamed to when `document_output_dir` was set. `content` is empty in memory when this is populated. |
| `content_base64` | `str \| None` | `None` | Base64-encoded copy of `content`, populated only when `document_content_encoding` was set to `Base64`. |
...
| `screenshot_base64` | `str \| None` | `None` | Base64-encoded PNG screenshot taken after all actions. Populated only when a `PageAction.Screenshot` action actually ran, so callers that never request a screenshot do not pay the encoding cost. |
...
|-------|------|---------|-------------|
| `status_code` | `int` | — | The HTTP status code of the response. |
| `final_url` | `str` | — | The final URL after following all redirects. |
| `content_type` | `str` | — | The Content-Type header value. |
| `html` | `str` | — | The HTML body of the response. |
| `body_size` | `int` | — | The size of the response body in bytes. |
| `metadata` | `PageMetadata` | — | Extracted metadata from the page. |
...
| `x_robots_tag` | `str \| None` | `None` | The X-Robots-Tag header values, joined with `, ` when the response sent more than one. |
| `is_pdf` | `bool` | — | Whether the content is a PDF. |
...
| `extracted_data` | `dict\[str, Any\] \| None` | `None` | Structured data extracted by LLM. Populated when extraction is configured. |
| `extraction_meta` | `ExtractionMeta \| None` | `None` | Metadata about the LLM extraction pass (cost, tokens, model). |
| `screenshot_base64` | `str \| None` | `None` | Base64-encoded PNG screenshot of the page. Populated only when `CrawlConfig.capture_screenshot` was enabled for this request, so callers that never requested a screenshot do not pay the encoding cost. |
| `downloaded_document` | `DownloadedDocument \| None` | `None` | Downloaded non-HTML document (PDF, DOCX, image, code, etc.). |
...
| Value | Description |
|-------|-------------|
| `CLICK` | Click on an element matching the given CSS selector. — Fields: `selector`: `str` |
| `TYPE_TEXT` | Type text into an element matching the given CSS selector. — Fields: `selector`: `str`, `text`: `str` |
| `PRESS` | Press a keyboard key (e.g. "Enter", "Tab", "Escape"). — Fields: `key`: `str` |
| `SCROLL` | Scroll the page or a specific element. — Fields: `direction`: `ScrollDirection`, `selector`: `str`, `amount`: `int` |
| `WAIT` | Wait for a duration or for an element to appear. — Fields: `milliseconds`: `int`, `selector`: `str` |
| `SCREENSHOT` | Take a screenshot of the current page. — Fields: `full_page`: `bool` |
| `EXECUTE_JS` | Execute arbitrary JavaScript in the page context. **Safety:** The script runs with full page privileges in the browser context. Only execute scripts from trusted sources. — Fields: `script`: `str` |
...
|-----------|-------------|
| `NotFoundError(CrawlError)` | The requested page was not found (HTTP 404). |
| `UnauthorizedError(CrawlError)` | The request was unauthorized (HTTP 401). |
| `ForbiddenError(CrawlError)` | The request was forbidden (HTTP 403). |
| `WafBlockedError(CrawlError)` | The request was blocked by a WAF or bot protection (HTTP 403 with WAF indicators). `vendor` is the lowercase identifier of the detected WAF (e.g. "cloudflare", "datadome"). When the engine cannot identify the vendor, it uses "unknown". `message` is the freeform description for logs and human readers. The stable error tag remains `forbidden: waf/blocked: MESSAGE` so existing log-grep patterns and cross-language bindings continue to work; vendor is surfaced separately for structured consumers. |
| `CrawlTimeoutError(CrawlError)` | The request timed out. |
| `RateLimitedError(CrawlError)` | The request was rate-limited (HTTP 429). |
| `ServerError(CrawlError)` | A server error occurred (HTTP 5xx). |
| `BadGatewayError(CrawlError)` | A bad gateway error occurred (HTTP 502). |
| `GoneError(CrawlError)` | The resource is permanently gone (HTTP 410). |
| `CrawlConnectionError(CrawlError)` | A connection error occurred. |
| `DnsError(CrawlError)` | A DNS resolution error occurred. |
| `SslError(CrawlError)` | An SSL/TLS error occurred. |
| `DataLossError(CrawlError)` | Data was lost or truncated during transfer. |
| `BrowserError(CrawlError)` | The browser failed to launch, connect, or navigate. |
| `BrowserTimeoutError(CrawlError)` | The browser page load or rendering timed out. |
| `InvalidConfigError(CrawlError)` | The provided configuration is invalid. |
| `UnsupportedError(CrawlError)` | The requested capability is not supported by the active backend or build. |
| `SsrfPolicyViolationError(CrawlError)` | A URL was rejected by SSRF policy (private IP, metadata, disallowed scheme, etc). |
| `OtherError(CrawlError)` | An unclassified error occurred. |
...
| Exception | Description |
|-----------|-------------|
| `DeniedByPolicyError(SsrfError)` | URL denied by SSRF policy: private IP, metadata IP, etc. |
| `NotOnAllowlistError(SsrfError)` | Host not on allowlist when an allowlist is configured. |
| `InvalidCidrError(SsrfError)` | Allowlist entry is not a parseable CIDR block. |
| `DnsResolutionFailedError(SsrfError)` | DNS resolution failed for hostname. |
| `InvalidUrlError(SsrfError)` | Invalid URL format. |
| `DisallowedSchemeError(SsrfError)` | URL scheme not in allowlist (e.g., `ftp://` when only `http`/`https` allowed). |
| `TooManyRedirectsError(SsrfError)` | Too many HTTP redirects encountered during validation. |
