...
<!-- alef:hash:f192a4d4558fc665d6ba4f289a6fc1b782dad3b5e29041e1d3c35ee871e9cd33 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## C# API Reference <span class="version-badge">v1.3.3</span>
...
```csharp
var result = await Scrape(new CrawlEngineHandle(), "value");
```
...
```csharp
var result = await Crawl(new CrawlEngineHandle(), "value");
```
...
```csharp
var result = await MapUrls(new CrawlEngineHandle(), "value");
```
...
```csharp
var result = await Interact(new CrawlEngineHandle(), "value", new List<object>());
```
...
```csharp
var result = await BatchScrape(new CrawlEngineHandle(), new List<object>());
```
...
```csharp
var result = await BatchCrawl(new CrawlEngineHandle(), new List<object>());
```
