...
<!-- alef:hash:df033af81b94d796db435426d68ef4608619ed8410be1790da07978e84ee03f3 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Go API Reference <span class="version-badge">v1.8.0</span>
...
| `CompletedCount` | `uint` | — | Number of seed URLs whose crawl succeeded (`error` is `nil`). |
...
| `CompletedCount` | `uint` | — | Number of URLs whose scrape succeeded (`error` is `nil`). |
...
| `RetryCodes` | `\[\]uint16` | `nil` | HTTP status codes that should trigger a retry. When empty, every rate limit, server error, bad gateway and timeout is retried. When set, only a failure whose status is listed is retried, so a timeout without a response is not. |
...
| `RateLimitJitterRatio` | `float64` | `0` | Fraction of the per-domain rate-limit delay to randomly jitter by, in `[0.0, 1.0]`. `0.0` (the default) applies no jitter and preserves the previous fixed-interval behaviour; `0.1` jitters the delay by up to ±10%. |
...
```go
func (o *CrawlResult) UniqueNormalizedUrls() uint
```
...
| `AssetCategory` | `AssetCategory` | `AssetCategoryImage` | The category of the asset. |
...
| `FeedType` | `FeedType` | `FeedTypeRss` | The type of feed. |
...
| `Source` | `ImageSource` | `ImageSourceImg` | The source of the image reference. |
...
| `LinkType` | `LinkType` | `LinkTypeInternal` | The classification of the link. |
...
| `WordCount` | `*uint` | `nil` | Computed word count of the page body text. |
...
| `XRobotsTag` | `*string` | `nil` | The X-Robots-Tag header values, joined with `, ` when the response sent more than one. |
| `IsPdf` | `bool` | — | Whether the content is a PDF. |
...
| Variant | Description |
|---------|-------------|
| `ErrNotFound` | The requested page was not found (HTTP 404). |
| `ErrUnauthorized` | The request was unauthorized (HTTP 401). |
| `ErrForbidden` | The request was forbidden (HTTP 403). |
| `ErrWafBlocked` | The request was blocked by a WAF or bot protection (HTTP 403 with WAF indicators). `vendor` is the lowercase identifier of the detected WAF (e.g. "cloudflare", "datadome"). When the engine cannot identify the vendor, it uses "unknown". `message` is the freeform description for logs and human readers. The stable error tag remains `forbidden: waf/blocked: MESSAGE` so existing log-grep patterns and cross-language bindings continue to work; vendor is surfaced separately for structured consumers. |
| `ErrTimeout` | The request timed out. |
| `ErrRateLimited` | The request was rate-limited (HTTP 429). |
| `ErrServerError` | A server error occurred (HTTP 5xx). |
| `ErrBadGateway` | A bad gateway error occurred (HTTP 502). |
| `ErrGone` | The resource is permanently gone (HTTP 410). |
| `ErrConnection` | A connection error occurred. |
| `ErrDns` | A DNS resolution error occurred. |
| `ErrSsl` | An SSL/TLS error occurred. |
| `ErrDataLoss` | Data was lost or truncated during transfer. |
| `ErrBrowserError` | The browser failed to launch, connect, or navigate. |
| `ErrBrowserTimeout` | The browser page load or rendering timed out. |
| `ErrInvalidConfig` | The provided configuration is invalid. |
| `ErrUnsupported` | The requested capability is not supported by the active backend or build. |
| `ErrSsrfPolicyViolation` | A URL was rejected by SSRF policy (private IP, metadata, disallowed scheme, etc). |
| `ErrOther` | An unclassified error occurred. |
...
| Variant | Description |
|---------|-------------|
| `ErrDeniedByPolicy` | URL denied by SSRF policy: private IP, metadata IP, etc. |
| `ErrNotOnAllowlist` | Host not on allowlist when an allowlist is configured. |
| `ErrInvalidCidr` | Allowlist entry is not a parseable CIDR block. |
| `ErrDnsResolutionFailed` | DNS resolution failed for hostname. |
| `ErrInvalidUrl` | Invalid URL format. |
| `ErrDisallowedScheme` | URL scheme not in allowlist (e.g., `ftp://` when only `http`/`https` allowed). |
| `ErrTooManyRedirects` | Too many HTTP redirects encountered during validation. |
