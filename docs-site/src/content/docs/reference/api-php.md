...
<!-- alef:hash:db3307754cdfe8d426c23e39fddd7d26d604ad654913f2c0b9e87a71514f6cb8 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## PHP API Reference <span class="version-badge">v1.8.0</span>
...
| `retryCount` | `int` | `0` | Number of retry attempts for failed requests. Bounded by `MAX_RETRY_COUNT`. |
| `retryCodes` | `array<int>` | `[]` | HTTP status codes that should trigger a retry. When empty, every rate limit, server error, bad gateway and timeout is retried. When set, only a failure whose status is listed is retried, so a timeout without a response is not. |
...
| `rateLimitJitterRatio` | `float` | `0` | Fraction of the per-domain rate-limit delay to randomly jitter by, in `[0.0, 1.0]`. `0.0` (the default) applies no jitter and preserves the previous fixed-interval behaviour; `0.1` jitters the delay by up to ±10%. |
...
| `redirectCount` | `int` | — | Redirect hops taken to reach `final_url` from `url`. |
