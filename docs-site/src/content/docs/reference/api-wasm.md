...
<!-- alef:hash:c1af94a3174b65d962c8e3cfe162bb1513e387392fd5480994d0e17f50737ccc -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## WebAssembly API Reference <span class="version-badge">v1.8.0</span>
...
```typescript
const result = createEngine(new CrawlConfig());
```
...
```typescript
const result = await scrape(new CrawlEngineHandle(), "value");
```
...
```typescript
const result = await crawl(new CrawlEngineHandle(), "value");
```
...
```typescript
const result = await mapUrls(new CrawlEngineHandle(), "value");
```
...
```typescript
const result = await interact(new CrawlEngineHandle(), "value", []);
```
...
```typescript
const result = await batchScrape(new CrawlEngineHandle(), []);
```
...
```typescript
const result = await batchCrawl(new CrawlEngineHandle(), []);
```
...
| `retryCount` | `number` | `0` | Number of retry attempts for failed requests. Bounded by `MAX_RETRY_COUNT`. |
| `retryCodes` | `Array<number>` | `[]` | HTTP status codes that should trigger a retry. When empty, every rate limit, server error, bad gateway and timeout is retried. When set, only a failure whose status is listed is retried, so a timeout without a response is not. |
| `retryInitialDelayMs` | `number` | `100` | Initial delay, in milliseconds, before the first retry. Doubled on each subsequent attempt (capped at `retry_max_delay_ms`). Defaults to 100ms. |
| `retryMaxDelayMs` | `number` | `60000` | Upper bound, in milliseconds, on the exponential retry backoff. Defaults to 60s. |
| `rateLimitJitterRatio` | `number` | `0` | Fraction of the per-domain rate-limit delay to randomly jitter by, in `[0.0, 1.0]`. `0.0` (the default) applies no jitter and preserves the previous fixed-interval behaviour; `0.1` jitters the delay by up to ±10%. |
...
| `redirectCount` | `number` | — | Redirect hops taken to reach `final_url` from `url`. |
...
| `contentPath` | `string \| null` | `null` | Filesystem path the document was streamed to when `document_output_dir` was set. `content` is empty in memory when this is populated. |
| `contentBase64` | `string \| null` | `null` | Base64-encoded copy of `content`, populated only when `document_content_encoding` was set to `Base64`. |
...
| `screenshotBase64` | `string \| null` | `null` | Base64-encoded PNG screenshot taken after all actions. Populated only when a `PageAction.Screenshot` action actually ran, so callers that never request a screenshot do not pay the encoding cost. |
...
| `xRobotsTag` | `string \| null` | `null` | The X-Robots-Tag header values, joined with `, ` when the response sent more than one. |
| `isPdf` | `boolean` | — | Whether the content is a PDF. |
...
| `extractedData` | `unknown \| null` | `null` | Structured data extracted by LLM. Populated when extraction is configured. |
| `extractionMeta` | `ExtractionMeta \| null` | `null` | Metadata about the LLM extraction pass (cost, tokens, model). |
| `screenshotBase64` | `string \| null` | `null` | Base64-encoded PNG screenshot of the page. Populated only when `CrawlConfig.capture_screenshot` was enabled for this request, so callers that never requested a screenshot do not pay the encoding cost. |
| `downloadedDocument` | `DownloadedDocument \| null` | `null` | Downloaded non-HTML document (PDF, DOCX, image, code, etc.). |
