...
<!-- alef:hash:d5ce168e527ec0b060c61273accd41ac820393ee18d80698b30c7a396c050204 -->
...
| `pathPatternsMatchUrl` | `boolean` | `false` | Whether `include_paths`/`exclude_paths` match against the full URL, `scheme://host[:port]/path?query`, so a pattern can scope by host. Defaults to `false`. When `true` it takes precedence over `Self.path_patterns_match_query`: the query is part of the full URL whatever that flag says. The matched text never contains a `user:password@`, a fragment or a default port, and the host is in punycode (`bücher.de` is matched as `xn--bcher-kva.de`). |
