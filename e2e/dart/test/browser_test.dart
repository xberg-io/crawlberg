...
// alef:hash:5c7dcd5c29566a6a452bc95c9df72194c41919f1c32ba2bb521e1b39941c7154
...
void main() {
  ...
  test(
    ...
    () async {
      final engineConfig = await createCrawlConfigFromJson(
        json: r'{"browser":{"mode":"always"},"max_depth":1,"respect_robots_txt":false}',
      );
      final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('Crawl with browser mode \'auto\' falls back to browser when encountering WAF 403', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"browser":{"mode":"auto"},"max_depth":1,"respect_robots_txt":false}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('Browser extra_wait adds additional time after network_idle to ensure all async operations complete', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"browser":{"extra_wait":200,"mode":"always","wait":"network_idle"}}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('Browser wait strategy \'selector\' waits for specific CSS selector before considering page loaded', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"browser":{"mode":"always","wait":"selector","wait_selector":"#content"}}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
