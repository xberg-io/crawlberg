...
// alef:hash:6e9ff63d2da7dbc1152aff683f41008d10af5f6a1a154e20bc078afe20bf83b8
...
void main() {
  ...
  test('Concurrent crawl respects max_pages budget', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"max_concurrent":4,"max_depth":1,"max_pages":3,"respect_robots_txt":false}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('Sends custom headers on all crawl requests', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"custom_headers":{"Accept-Language":"en-US","X-Custom-Header":"test-value"},"max_depth":1,"respect_robots_txt":false}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('Skips URLs matching the exclude path pattern', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"exclude_paths":["/admin/.*"],"max_depth":1,"respect_robots_txt":false}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('Only follows URLs matching the include path pattern', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"include_paths":["/blog/.*"],"max_depth":1,"respect_robots_txt":false}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
  ...
  test('max_links_per_page caps the number of links enqueued from a single page', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json:
          r'{"max_links_per_page":2,"max_pages":10,"respect_robots_txt":false}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
    final url = _fixtureUrl("crawl_max_links_per_page");
    final result = await CrawlbergBridge.crawl(engine, url);
    expect(result.pages.length, lessThanOrEqualTo(3));
  });
  ...
  test('Stays on exact domain and skips subdomain links', () async {
    final engineConfig = await createCrawlConfigFromJson(
      json: r'{"allow_subdomains":false,"max_depth":1,"respect_robots_txt":false,"stay_on_domain":true}',
    );
    final engine = await CrawlbergBridge.createEngine(config: engineConfig);
