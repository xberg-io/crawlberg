...
// alef:hash:58bacb101656cce86f27ac467d3c364b55d284281833f2bddd0523ea6d648e80
...
public class MarkdownTest {
    ...
    void testMarkdownBasicConversion() throws Exception {
        ...
        var result = Crawlberg.scrape(engine, url);
        assertEquals(200, result.statusCode());
        assertEquals("Test", java.util.Optional.ofNullable(result.metadata().title()).map(java.util.Objects::toString).orElse(""));
        assertFalse(result.html().isEmpty(), "expected non-empty value");
        assertFalse(result.markdown().content().isEmpty(), "expected non-empty value");
        assertTrue(result.markdown().content().contains("Hello World"), "expected to contain: " + "Hello World");
    ...
    void testMarkdownHeadingsAndParagraphs() throws Exception {
        ...
        var result = Crawlberg.scrape(engine, url);
        assertFalse(result.markdown().content().isEmpty(), "expected non-empty value");
        assertTrue(result.markdown().content().contains("Main Title"), "expected to contain: " + "Main Title");
    ...
    void testMarkdownLinksConverted() throws Exception {
        ...
        assertFalse(result.html().isEmpty(), "expected non-empty value");
        assertFalse(result.markdown().content().isEmpty(), "expected non-empty value");
        assertTrue(result.markdown().content().contains("Example"), "expected to contain: " + "Example");
    ...
    @Test
    void testMarkdownNoFrontmatter() throws Exception {
        ...
        var engine = Crawlberg.createEngine(engineConfig);
        String url = System.getProperty("mockServerUrl", System.getenv("MOCK_SERVER_URL")) + "/fixtures/markdown_no_frontmatter";
        var result = Crawlberg.scrape(engine, url);
        assertEquals(200, result.statusCode());
        assertEquals("Test", java.util.Optional.ofNullable(result.metadata().title()).map(java.util.Objects::toString).orElse(""));
        assertFalse(result.markdown().content().contains("---"), "expected NOT to contain: " + "---");
        assertTrue(result.markdown().content().contains("Hello World"), "expected to contain: " + "Hello World");
