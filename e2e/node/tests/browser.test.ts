...
// alef:hash:8c2e3427e67727c5effd25d1049fb2c5f95f4a7e5b975dfac8c231e3352289ce
// To regenerate: alef e2e generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { scrape, crawl, createEngine, BrowserMode, BrowserWait } from "@xberg-io/crawlberg";
...
describe("browser", () => {
	...
	it("browser_crawl_mode_always: Crawl with browser mode 'always' follows links using browser rendering", async () => {
		...
		const engine = createEngine(engineConfig);
		const url = process.env.MOCK_SERVER_BROWSER_CRAWL_MODE_ALWAYS ?? `${process.env.MOCK_SERVER_URL}/fixtures/browser_crawl_mode_always`;
	...
	it("browser_crawl_waf_fallback: Crawl with browser mode 'auto' falls back to browser when encountering WAF 403", async () => {
		...
		const engine = createEngine(engineConfig);
		const url = process.env.MOCK_SERVER_BROWSER_CRAWL_WAF_FALLBACK ?? `${process.env.MOCK_SERVER_URL}/fixtures/browser_crawl_waf_fallback`;
		const result = await crawl(engine, url);
