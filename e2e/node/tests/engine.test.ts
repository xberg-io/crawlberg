...
import { scrape, crawl, mapUrls, crawlStream, createEngine } from "@xberg-io/crawlberg";
...
describe("engine", () => {
  ...
  it("engine_scrape_basic: CrawlEngine with defaults scrapes a page identically to the free function", async () => {
    const engine = createEngine(null);
    const url =
      process.env.MOCK_SERVER_ENGINE_SCRAPE_BASIC ?? `${process.env.MOCK_SERVER_URL}/fixtures/engine_scrape_basic`;
  ...
  it("engine_stream_basic: CrawlEngine with defaults streams events like the free function", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ENGINE_STREAM_BASIC ?? `${process.env.MOCK_SERVER_URL}/fixtures/engine_stream_basic`;
