...
describe("cookies", () => {
  it("cookies_per_domain: Isolates cookies per domain during crawl", async () => {
    ...
    expect(result.cookies.some((item) => _alefE2eItemTexts(item).some((text) => text.includes("domain_cookie")))).toBe(
      true,
    );
  }, 30000);
  it("cookies_persistence: Maintains cookies across multiple crawl requests", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_COOKIES_PERSISTENCE ?? `${process.env.MOCK_SERVER_URL}/fixtures/cookies_persistence`;
    const result = await crawl(engine, url);
