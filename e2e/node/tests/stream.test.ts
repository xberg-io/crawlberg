...
describe("stream", () => {
  ...
  it("crawl_stream_events: Crawl stream produces page and complete events", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_CRAWL_STREAM_EVENTS ?? `${process.env.MOCK_SERVER_URL}/fixtures/crawl_stream_events`;
  ...
  it("stream_depth_crawl: Stream produces events for multi-depth crawl with link following", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_STREAM_DEPTH_CRAWL ?? `${process.env.MOCK_SERVER_URL}/fixtures/stream_depth_crawl`;
    const stream = await crawlStream(engine, url);
    const chunks: any[] = [];
    for await (const _chunk of stream) {
      chunks.push(_chunk);
  ...
  it("stream_event_ordering: Stream ensures complete event arrives after all page events", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_STREAM_EVENT_ORDERING ?? `${process.env.MOCK_SERVER_URL}/fixtures/stream_event_ordering`;
    const stream = await crawlStream(engine, url);
    const chunks: any[] = [];
    for await (const _chunk of stream) {
      chunks.push(_chunk);
  ...
  it("stream_large_crawl: Stream handles crawl of 5+ pages with multiple events", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_STREAM_LARGE_CRAWL ?? `${process.env.MOCK_SERVER_URL}/fixtures/stream_large_crawl`;
    const stream = await crawlStream(engine, url);
    const chunks: any[] = [];
    for await (const _chunk of stream) {
      chunks.push(_chunk);
