// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:4c2045d3183bcc9b269e975307e912311c74ae0430f589326663c88bfcccbad0
// To regenerate: alef e2e generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { scrape, createEngine } from "@xberg-io/crawlberg";

process.env.CRAWLBERG_ALLOW_PRIVATE_NETWORK ??= "true";


async function _alefE2eDecompressAndParseJson(response: Response): Promise<unknown> {
	const contentEncoding = response.headers.get("content-encoding");
	let bytes = new Uint8Array(await response.arrayBuffer());

	// The runtime's own fetch (undici on Node, and the browser equivalent) decodes
	// `content-encoding` transparently and leaves the header in place, so by the time the
	// body reaches here it is usually already plain text. Decoding unconditionally on the
	// header alone therefore fails ("incorrect header check") against a server that really
	// did compress. Decode only when the payload still carries the encoding: gzip announces
	// itself with the 0x1f8b magic pair; brotli has no magic, so it is attempted and the
	// already-decoded case falls back to the bytes as received.
	if (contentEncoding === "gzip" && bytes.length >= 2 && bytes[0] === 0x1f && bytes[1] === 0x8b) {
		const { gunzipSync } = await import("zlib");
		bytes = new Uint8Array(gunzipSync(bytes));
	} else if (contentEncoding === "br") {
		const { brotliDecompressSync } = await import("zlib");
		try {
			bytes = new Uint8Array(brotliDecompressSync(bytes));
		} catch {
			// Already decoded by the runtime.
		}
	}

	const text = new TextDecoder().decode(bytes);
	return JSON.parse(text);
}

function _alefE2eText(value: unknown): string {
	return value == null ? "" : String(value);
}

function _alefE2eItemTexts(item: unknown): string[] {
	if (item == null || typeof item !== "object") {
		return [_alefE2eText(item)];
	}
	const record = item as Record<string, unknown>;
	const itemsText = Array.isArray(record.items) ? record.items.map(_alefE2eText).join(" ") : "";
	return [_alefE2eText(item), _alefE2eText(record.kind), _alefE2eText(record.name), _alefE2eText(record.source), _alefE2eText(record.alias), _alefE2eText(record.text), _alefE2eText(record.signature), itemsText];
}

function _alefE2eFormatMetadataDisplay(fm: unknown): string {
	if (fm == null) return "";
	if (typeof fm !== "object") return String(fm);
	const record = fm as Record<string, unknown>;
	const formatType = record.format_type;

	// FormatMetadata is internally tagged, so serde merges the variant payload's own fields as
	// siblings of the tag rather than nesting them: { format_type: 'image', format: 'PNG', ... }.
	// Read `format` off the record directly. Deliberately NO nested (`record.image`) fallback --
	// a binding that regresses to the pre-flatten shape must fail this assertion loudly instead
	// of silently degrading to the variant name below.
	if (formatType === "image" && typeof record.format === "string") {
		return record.format;
	}

	// Fallback: return format_type variant name
	if (typeof record.format_type === "string") return record.format_type;
	return "";
}


describe("content", () => {

	it("content_204_no_content: Handles 204 No Content response gracefully", async () => {
		const engine = createEngine(null);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_204_no_content`;
		const result = await scrape(engine, url);
    expect(result.statusCode).toBe(204);
    expect((result.html ?? "").length).toBe(0);

	}, 30000);

	it("content_charset_iso8859: Handles ISO-8859-1 encoded page correctly", async () => {
		const engineConfig = { respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_charset_iso8859`;
		const result = await scrape(engine, url);
    expect((result.detectedCharset ?? "")).toBe("iso-8859-1");

	}, 30000);

	it("content_default_preset: Default preprocessing keeps the footer that the aggressive preset drops", async () => {
		const engineConfig = { respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_default_preset`;
		const result = await scrape(engine, url);
    expect(result.markdown?.content ?? "").toContain("Main Article Title");
    expect(result.markdown?.content ?? "").toContain("Footer content");
    expect(result.markdown?.content).not.toContain("Sidebar content");

	}, 30000);

	it("content_empty_body: Handles 200 response with empty body gracefully", async () => {
		const engineConfig = { respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_empty_body`;
		const result = await scrape(engine, url);
    expect(result.statusCode).toBe(200);

	}, 30000);

	it("content_gzip_compressed: Handles response with Accept-Encoding gzip negotiation", async () => {
		const engineConfig = { respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_gzip_compressed`;
		const result = await scrape(engine, url);
    {
        const _v = result.html;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }
    expect(result.statusCode).toBe(200);

	}, 30000);

	it("content_large_page_limit: Respects max body size limit and truncates or skips oversized pages", async () => {
		const engineConfig = { maxBodySize: 1024, respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_large_page_limit`;
		const result = await scrape(engine, url);
    expect(result.bodySize).toBeLessThan(1025);

	}, 30000);

	it("content_main_only: Extracts content with aggressive preprocessing, excluding nav, sidebar, footer", async () => {
		const engineConfig = { content: { preprocessingPreset: "aggressive" }, respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_main_only`;
		const result = await scrape(engine, url);
    expect(result.markdown?.content ?? "").toContain("Main Article Title");
    expect(result.markdown?.content).not.toContain("Footer content");
    expect(result.markdown?.content).not.toContain("Sidebar content");

	}, 30000);

	it("content_non_ascii_body_limit: Truncates a non-ASCII body at a UTF-8 character boundary", async () => {
		const engineConfig = { maxBodySize: 1, respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_non_ascii_body_limit`;
		const result = await scrape(engine, url);
    expect(result.statusCode).toBe(200);
    expect(result.bodySize).toBe(0);

	}, 30000);

	it("content_pdf_no_extension: Detects PDF content by Content-Type header when URL has no .pdf extension", async () => {
		const engineConfig = { respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_pdf_no_extension`;
		const result = await scrape(engine, url);
    expect(result.isPdf).toBe(true);

	}, 30000);

	it("content_remove_tags: Removes specified HTML elements by CSS selector before processing", async () => {
		const engineConfig = { removeTags: ["nav", "aside", "footer"], respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_remove_tags`;
		const result = await scrape(engine, url);
    {
        const _v = result.html;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }

	}, 30000);

	it("content_utf8_bom: Handles UTF-8 content with BOM marker correctly", async () => {
		const engineConfig = { respectRobotsTxt: false };
		const engine = createEngine(engineConfig);
		const url = `${process.env.MOCK_SERVER_URL}/fixtures/content_utf8_bom`;
		const result = await scrape(engine, url);
    expect((result.detectedCharset ?? "")).toBe("utf-8");
    {
        const _v = result.html;
        if (typeof _v === "string" || Array.isArray(_v)) {
            expect(_v.length).toBeGreaterThan(0);
        } else {
            expect(_v).toBeDefined();
            expect(_v).not.toBeNull();
        }
    }

	}, 30000);

});
