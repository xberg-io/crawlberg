...
RSpec.describe "encoding" do
  it "encoding_double_encoded: Handles double-encoded URL characters (%25C3%25B6)" do
    ...
    url = ENV.fetch("MOCK_SERVER_ENCODING_DOUBLE_ENCODED", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/encoding_double_encoded"
  ...
  it "encoding_percent_encoded_path: Handles percent-encoded spaces and characters in URL paths" do
    engine = Crawlberg.create_engine(nil)
    url = ENV.fetch("MOCK_SERVER_ENCODING_PERCENT_ENCODED_PATH", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/encoding_percent_encoded_path"
    result = Crawlberg.scrape(engine, url)
    expect(result.html.to_s).not_to(be_empty)
