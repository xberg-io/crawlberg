...
RSpec.describe "cookies" do
  ...
  it "cookies_per_domain: Isolates cookies per domain during crawl" do
    ...
    url = ENV.fetch("MOCK_SERVER_COOKIES_PER_DOMAIN", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/cookies_per_domain"
    result = Crawlberg.crawl(engine, url)
    expect(result.cookies.length).to(eq(1))
    expect(result.cookies.any? { |item| alef_e2e_item_texts(item).any? { |t| t.include?("domain_cookie") } }).to(
      be(true)
    )
  end
  ...
  it "cookies_persistence: Maintains cookies across multiple crawl requests" do
    engine_config = {"cookies_enabled" => true, "max_depth" => 1, "respect_robots_txt" => false}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = ENV.fetch("MOCK_SERVER_COOKIES_PERSISTENCE", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/cookies_persistence"
    result = Crawlberg.crawl(engine, url)
  ...
  it "cookies_set_cookie_response: Respects Set-Cookie header from server responses" do
    engine_config = {"cookies_enabled" => true, "max_depth" => 1, "respect_robots_txt" => false}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = ENV.fetch("MOCK_SERVER_COOKIES_SET_COOKIE_RESPONSE", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/cookies_set_cookie_response"
    result = Crawlberg.crawl(engine, url)
