# This file is auto-generated by alef — DO NOT EDIT.
# alef:hash:17f9f8de28107eff953439f136be0939f7d878b9c0bfeba5966d724bf5eb3056
# To regenerate: alef generate
# To verify freshness: alef verify --exit-code
# frozen_string_literal: true

require "spec_helper"
require "crawlberg"
require "json"

RSpec.describe "browser" do
  it "browser_config_auto_no_feature: Browser mode 'never' prevents browser use even when JS render hint is set" do
    engine_config = {"browser" => {"mode" => "never"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_config_auto_no_feature"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(false))
  end

  it "browser_config_never_mode: Browser mode 'never' prevents browser fallback even for SPA shell content" do
    engine_config = {"browser" => {"mode" => "never"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_config_never_mode"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(false))
  end

  it "browser_crawl_mode_always: Crawl with browser mode 'always' follows links using browser rendering" do
    engine_config = {"browser" => {"mode" => "always"}, "max_depth" => 1, "respect_robots_txt" => false}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = ENV.fetch("MOCK_SERVER_BROWSER_CRAWL_MODE_ALWAYS", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_crawl_mode_always"
    result = Crawlberg.crawl(engine, url)
    expect(result.pages.length).to(be >= 2)
    # skipped: field 'browser_used' not available on result type
  end

  it "browser_crawl_waf_fallback: Crawl with browser mode 'auto' falls back to browser when encountering WAF 403" do
    engine_config = {"browser" => {"mode" => "auto"}, "max_depth" => 1, "respect_robots_txt" => false}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = ENV.fetch("MOCK_SERVER_BROWSER_CRAWL_WAF_FALLBACK", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_crawl_waf_fallback"
    result = Crawlberg.crawl(engine, url)
    expect(result.pages.length).to(be >= 1)
  end

  it "browser_detect_minimal_page: Does NOT flag a short but real content page as needing JS rendering" do
    engine = Crawlberg.create_engine(nil)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_minimal_page"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(false))
    expect(result.browser_used).to(be(false))
  end

  it(
    "browser_detect_next_empty: Detects Next.js page with __NEXT_DATA__ but no rendered content as needing JS rendering"
  ) do
    engine_config = {"browser" => {"mode" => "never"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_next_empty"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(false))
  end

  it "browser_detect_next_rendered: Does NOT flag Next.js page with full SSR content as needing JS rendering" do
    engine = Crawlberg.create_engine(nil)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_next_rendered"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.html.to_s).not_to(be_empty)
    expect(result.js_render_hint).to(be(false))
    expect(result.browser_used).to(be(false))
  end

  it "browser_detect_normal_page: Does NOT flag a normal server-rendered page as needing JS rendering" do
    engine = Crawlberg.create_engine(nil)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_normal_page"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(false))
    expect(result.browser_used).to(be(false))
  end

  it "browser_detect_nuxt_shell: Detects Nuxt SPA shell with empty #__nuxt div as needing JS rendering" do
    engine_config = {"browser" => {"mode" => "never"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_nuxt_shell"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(false))
  end

  it "browser_detect_react_shell: Detects React SPA shell with empty #root div as needing JS rendering" do
    engine_config = {"browser" => {"mode" => "never"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_react_shell"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.html.to_s).not_to(be_empty)
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(false))
  end

  it "browser_detect_vue_shell: Detects Vue SPA shell with empty #app div as needing JS rendering" do
    engine_config = {"browser" => {"mode" => "never"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_detect_vue_shell"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(false))
  end

  it(
    "browser_extra_wait: Browser extra_wait adds additional time after network_idle to ensure all async operations complete"
  ) do
    engine_config = {"browser" => {"extra_wait" => 200, "mode" => "always", "wait" => "network_idle"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_extra_wait"
    result = Crawlberg.scrape(engine, url)
    expect(result.browser_used).to(be(true))
  end

  it "browser_fallback_spa_render: Browser auto re-fetches SPA shell when JS rendering is detected" do
    engine_config = {"browser" => {"mode" => "always"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_fallback_spa_render"
    result = Crawlberg.scrape(engine, url)
    expect(result.js_render_hint).to(be(true))
    expect(result.browser_used).to(be(true))
  end

  it(
    "browser_fallback_waf_blocked: Browser fallback is used when browser mode is always, simulating WAF-blocked scenario"
  ) do
    engine_config = {"browser" => {"mode" => "always"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_fallback_waf_blocked"
    result = Crawlberg.scrape(engine, url)
    expect(result.browser_used).to(be(true))
  end

  it "browser_mode_always: Browser mode 'always' uses browser even for normal server-rendered pages" do
    engine_config = {"browser" => {"mode" => "always"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_mode_always"
    result = Crawlberg.scrape(engine, url)
    expect(result.browser_used).to(be(true))
  end

  it "browser_profile_basic: Browser profile configuration persists and reuses browser state across crawl sessions" do
    engine_config = {"browser" => {"mode" => "always"}, "browser_profile" => "test-profile"}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_profile_basic"
    result = Crawlberg.scrape(engine, url)
    expect(result.browser_used).to(be(true))
    expect(result.status_code).to(eq(200))
  end

  it "browser_wait_fixed: Browser wait strategy 'fixed' waits for a specific duration after page navigation" do
    engine_config = {"browser" => {"extra_wait" => 100, "mode" => "always", "wait" => "fixed"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_wait_fixed"
    result = Crawlberg.scrape(engine, url)
    expect(result.browser_used).to(be(true))
    expect(result.status_code).to(eq(200))
  end

  it(
    "browser_wait_selector: Browser wait strategy 'selector' waits for specific CSS selector before considering page loaded"
  ) do
    engine_config = {"browser" => {"mode" => "always", "wait" => "selector", "wait_selector" => "#content"}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/browser_wait_selector"
    result = Crawlberg.scrape(engine, url)
    expect(result.browser_used).to(be(true))
    expect(result.status_code).to(eq(200))
  end
end
