...
# alef:hash:cd3ec8ec3232bd7b3faaea3069438ef1218d3b16f62d275fbc325444529b54a2
...
RSpec.describe 'browser' do
  ...
  it "browser_crawl_mode_always: Crawl with browser mode 'always' follows links using browser rendering" do
    ...
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = ENV.fetch('MOCK_SERVER_BROWSER_CRAWL_MODE_ALWAYS', nil) || "#{ENV.fetch('MOCK_SERVER_URL')}/fixtures/browser_crawl_mode_always"
  ...
  it "browser_crawl_waf_fallback: Crawl with browser mode 'auto' falls back to browser when encountering WAF 403" do
    ...
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = ENV.fetch('MOCK_SERVER_BROWSER_CRAWL_WAF_FALLBACK', nil) || "#{ENV.fetch('MOCK_SERVER_URL')}/fixtures/browser_crawl_waf_fallback"
    result = Crawlberg.crawl(engine, url)
  ...
  it 'browser_fallback_waf_blocked: Browser fallback is used when browser mode is always, simulating WAF-blocked scenario' do
    engine_config = { 'browser' => { 'mode' => 'always' } }
    engine = Crawlberg.create_engine(engine_config.to_json)
