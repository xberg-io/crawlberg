...
RSpec.describe "batch" do
  it "batch_crawl_basic: Batch crawl of 2 seed URLs with links to discover" do
    ...
    urls_base = ENV.fetch("MOCK_SERVER_BATCH_CRAWL_BASIC", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/batch_crawl_basic"
  ...
  it "batch_crawl_partial_failure: Batch crawl where one seed URL returns 404 error" do
    engine_config = {"max_depth" => 1, "respect_robots_txt" => false}
    engine = Crawlberg.create_engine(engine_config.to_json)
    urls_base = ENV.fetch("MOCK_SERVER_BATCH_CRAWL_PARTIAL_FAILURE", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/batch_crawl_partial_failure"
  ...
  it "batch_crawl_with_config: Batch crawl with max_depth=1 config verifying pages are discovered" do
    engine_config = {"max_depth" => 1, "respect_robots_txt" => false}
    engine = Crawlberg.create_engine(engine_config.to_json)
    urls_base = ENV.fetch("MOCK_SERVER_BATCH_CRAWL_WITH_CONFIG", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/batch_crawl_with_config"
    urls = ["/seed1", "/seed2"].map { |p| p.start_with?("http") ? p : "#{urls_base}#{p}" }
    result = Crawlberg.batch_crawl(engine, urls)
    expect(result.completed_count).to(eq(2))
    expect(result.failed_count).to(eq(0))
  ...
  it "batch_scrape_empty_urls_error: Batch scrape with empty batch_urls array returns error" do
    expect do
      ...
      urls_base = ENV.fetch("MOCK_SERVER_BATCH_SCRAPE_EMPTY_URLS_ERROR", nil) ||
        "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/batch_scrape_empty_urls_error"
  ...
  it "batch_scrape_with_config: Batch scrape with aggressive preprocessing configuration" do
    ...
    engine = Crawlberg.create_engine(engine_config.to_json)
    urls_base = ENV.fetch("MOCK_SERVER_BATCH_SCRAPE_WITH_CONFIG", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/batch_scrape_with_config"
  ...
  it "scrape_batch_basic: Batch scrape of multiple URLs all succeeding" do
    engine = Crawlberg.create_engine(nil)
    urls_base = ENV.fetch("MOCK_SERVER_SCRAPE_BATCH_BASIC", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/scrape_batch_basic"
  ...
  it "scrape_batch_partial_failure: Batch scrape with one URL failing returns partial results" do
    engine = Crawlberg.create_engine(nil)
    urls_base = ENV.fetch("MOCK_SERVER_SCRAPE_BATCH_PARTIAL_FAILURE", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/scrape_batch_partial_failure"
  ...
  it "scrape_batch_progress: Batch scrape of 2 URLs completes with 2 results" do
    engine = Crawlberg.create_engine(nil)
    urls_base = ENV.fetch("MOCK_SERVER_SCRAPE_BATCH_PROGRESS", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/scrape_batch_progress"
