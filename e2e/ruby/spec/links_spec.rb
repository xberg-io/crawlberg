...
# alef:hash:893a1a6f37d7be3535bd91afba7985ecbe7895852e21567b62d802a50a5f8650
...
RSpec.describe 'links' do
  ...
  it 'links_base_tag: Resolves relative URLs using base tag href' do
    ...
    expect(result.links.any? { |e| e.images[0].url.to_s.include?('example.com') }).to be true
  end
  ...
  it 'links_empty_href: Handles empty href attributes without errors' do
    ...
    expect(result.links.any? { |e| e.images[0].url.to_s.include?('/valid') }).to be true
  end
  ...
  it 'links_mailto_javascript_skip: Skips mailto:, javascript:, and tel: scheme links' do
    ...
    result = Crawlberg.scrape(engine, url)
    expect(result.links.length).to be > 0
    expect(result.links.any? { |e| e.images[0].url.to_s.include?('mailto:') }).to be false
  end
  ...
  it 'links_protocol_relative: Handles protocol-relative URLs (//example.com) correctly' do
    ...
    expect(result.links.any? { |e| e.images[0].url.to_s.include?('http://cdn.example.com/resource') }).to be true
    expect(result.links.any? { |e| e.images[0].url.to_s.include?('http://images.example.com/photo.jpg') }).to be true
  end
