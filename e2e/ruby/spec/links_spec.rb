...
# alef:hash:1d483c808562be034021269987a1ca86c5b6bc467d7a8da8754a60bfda9d79a7
...
RSpec.describe 'links' do
  ...
  it 'links_base_tag: Resolves relative URLs using base tag href' do
    ...
    expect(result.links.any? { |e| e.assets[0].url.to_s.include?('example.com') }).to be true
  end
  ...
  it 'links_empty_href: Handles empty href attributes without errors' do
    ...
    expect(result.links.any? { |e| e.assets[0].url.to_s.include?('/valid') }).to be true
  end
  ...
  it 'links_mailto_javascript_skip: Skips mailto:, javascript:, and tel: scheme links' do
    ...
    result = Crawlberg.scrape(engine, url)
    expect(result.links.length).to be > 0
    expect(result.links.any? { |e| e.assets[0].url.to_s.include?('mailto:') }).to be false
  end
  ...
  it 'links_protocol_relative: Handles protocol-relative URLs (//example.com) correctly' do
    ...
    expect(result.links.any? { |e| e.assets[0].url.to_s.include?('http://cdn.example.com/resource') }).to be true
    expect(result.links.any? { |e| e.assets[0].url.to_s.include?('http://images.example.com/photo.jpg') }).to be true
  end
