...
async def test_validation_ssrf_allowlisted_loopback() -> None:
    ...
    assert result.metadata.title.strip() == "Allowlisted"  # noqa: S101
...
async def test_validation_ssrf_ipv4_mapped_ipv6_denied() -> None:
    ...
    with pytest.raises(Exception):  # noqa: B017
        ...
        engine = create_engine(engine_config)
        url = os.environ["MOCK_SERVER_URL"] + "/fixtures/validation_ssrf_ipv4_mapped_ipv6_denied"
        await scrape(engine, url)
...
async def test_validation_ssrf_nat64_loopback_denied() -> None:
    ...
    with pytest.raises(Exception):  # noqa: B017
        engine_config = CrawlConfig(ssrf={"deny_private": True}, ssrf_deny_private_explicit=True)
        engine = create_engine(engine_config)
        url = os.environ["MOCK_SERVER_URL"] + "/fixtures/validation_ssrf_nat64_loopback_denied"
        await scrape(engine, url)
