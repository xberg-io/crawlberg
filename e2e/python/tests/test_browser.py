...
# alef:hash:4df2c636607b96f9e0b6b5b77f37146914aaa97d89d7b67b3d678fe31a28d20a
...
import pytest
from crawlberg import scrape, crawl, create_engine, CrawlConfig, BrowserConfig
...
async def test_browser_config_auto_no_feature() -> None:
    ...
    assert result["js_render_hint"] is True
...
async def test_browser_detect_minimal_page() -> None:
    ...
    result = await scrape(engine, url)
    assert result["status_code"] == 200
    assert result["js_render_hint"] is False
    assert result["browser_used"] is False
