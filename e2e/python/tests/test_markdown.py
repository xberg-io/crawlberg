...
# alef:hash:0349acaacdb39f8ab3b7b9cb386cb571d8638f426be5834287be19dd1896718a
...
import pytest
from crawlberg import ContentConfig, CrawlConfig, crawl, create_engine, scrape
...
@pytest.mark.asyncio
async def test_markdown_no_frontmatter() -> None:
    ...
    engine = create_engine(engine_config)
    url = os.environ["MOCK_SERVER_URL"] + "/fixtures/markdown_no_frontmatter"

    result = await scrape(engine, url)
    assert result.status_code == 200
    assert result.metadata.title == "Test"
    assert "---" not in (result.markdown.content if result.markdown else None)
    assert "Hello World" in (result.markdown.content if result.markdown else None)
