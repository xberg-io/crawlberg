/*
 * This file is auto-generated by alef — DO NOT EDIT.
 * alef:hash:a7f8da0e4076f4e06050d5065691ece9292c76cd1e0a77a508f461254674e37d
 * To regenerate: alef e2e generate
 * To verify freshness: alef verify
 */
/* E2e tests for category: content */

#include <assert.h>
#include <stdint.h>
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include "crawlberg.h"
#include "test_runner.h"

void test_content_204_no_content(void) {
    /* Handles 204 No Content response gracefully */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_204_NO_CONTENT");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_204_no_content", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    uint16_t status_code = cberg_scrape_result_status_code(result);
    char* html = cberg_scrape_result_html(result);
    assert(status_code == 204 && "equals assertion failed");
    assert((html == NULL || strlen(html) == 0 || strcmp(html, "[]") == 0 || strcmp(html, "{}") == 0) && "expected empty value");
    cberg_free_string(html);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_charset_iso8859(void) {
    /* Handles ISO-8859-1 encoded page correctly */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_CHARSET_ISO8859");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_charset_iso8859", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    char* content_detected_charset = cberg_scrape_result_detected_charset(result);
    assert(content_detected_charset != NULL && strcmp(content_detected_charset, "iso-8859-1") == 0 && "equals assertion failed");
    cberg_free_string(content_detected_charset);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_default_preset(void) {
    /* Default preprocessing keeps the footer that the aggressive preset drops */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_DEFAULT_PRESET");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_default_preset", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    CBERGAlefHandle markdown_handle = cberg_scrape_result_markdown(result);
    assert(markdown_handle != 0);
    char* markdown_content = cberg_markdown_result_content(markdown_handle);
    assert(markdown_content != NULL && strstr(markdown_content, "Main Article Title") != NULL && "expected to contain substring");
    assert(markdown_content != NULL && strstr(markdown_content, "Footer content") != NULL && "expected to contain substring");
    assert(markdown_content != NULL && strstr(markdown_content, "Sidebar content") == NULL && "expected non-null value without substring");
    cberg_free_string(markdown_content);
    cberg_markdown_result_free(markdown_handle);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_empty_body(void) {
    /* Handles 200 response with empty body gracefully */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_EMPTY_BODY");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_empty_body", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    uint16_t status_code = cberg_scrape_result_status_code(result);
    assert(status_code == 200 && "equals assertion failed");
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_gzip_compressed(void) {
    /* Handles response with Accept-Encoding gzip negotiation */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_GZIP_COMPRESSED");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_gzip_compressed", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    char* html = cberg_scrape_result_html(result);
    uint16_t status_code = cberg_scrape_result_status_code(result);
    assert(html != NULL && strlen(html) > 0 && strcmp(html, "[]") != 0 && strcmp(html, "{}") != 0 && "expected non-empty value");
    assert(status_code == 200 && "equals assertion failed");
    cberg_free_string(html);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_large_page_limit(void) {
    /* Respects max body size limit and truncates or skips oversized pages */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"max_body_size\":1024,\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_LARGE_PAGE_LIMIT");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_large_page_limit", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    uintptr_t content_body_size = cberg_scrape_result_body_size(result);
    assert(content_body_size < 1025 && "expected less than");
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_main_only(void) {
    /* Extracts content with aggressive preprocessing, excluding nav, sidebar, footer */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"content\":{\"preprocessing_preset\":\"aggressive\"},\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_MAIN_ONLY");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_main_only", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    CBERGAlefHandle markdown_handle = cberg_scrape_result_markdown(result);
    assert(markdown_handle != 0);
    char* markdown_content = cberg_markdown_result_content(markdown_handle);
    assert(markdown_content != NULL && strstr(markdown_content, "Main Article Title") != NULL && "expected to contain substring");
    assert(markdown_content != NULL && strstr(markdown_content, "Footer content") == NULL && "expected non-null value without substring");
    assert(markdown_content != NULL && strstr(markdown_content, "Sidebar content") == NULL && "expected non-null value without substring");
    cberg_free_string(markdown_content);
    cberg_markdown_result_free(markdown_handle);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_non_ascii_body_limit(void) {
    /* Truncates a non-ASCII body at a UTF-8 character boundary */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"max_body_size\":1,\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_NON_ASCII_BODY_LIMIT");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_non_ascii_body_limit", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    uint16_t status_code = cberg_scrape_result_status_code(result);
    uintptr_t content_body_size = cberg_scrape_result_body_size(result);
    assert(status_code == 200 && "equals assertion failed");
    assert(content_body_size == 0 && "equals assertion failed");
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_pdf_no_extension(void) {
    /* Detects PDF content by Content-Type header when URL has no .pdf extension */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_PDF_NO_EXTENSION");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_pdf_no_extension", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    int32_t content_is_pdf = cberg_scrape_result_is_pdf(result);
    assert(content_is_pdf == 1 && "equals assertion failed");
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_remove_tags(void) {
    /* Removes specified HTML elements by CSS selector before processing */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"remove_tags\":[\"nav\",\"aside\",\"footer\"],\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_REMOVE_TAGS");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_remove_tags", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    char* html = cberg_scrape_result_html(result);
    assert(html != NULL && strlen(html) > 0 && strcmp(html, "[]") != 0 && strcmp(html, "{}") != 0 && "expected non-empty value");
    cberg_free_string(html);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}

void test_content_utf8_bom(void) {
    /* Handles UTF-8 content with BOM marker correctly */
    CBERGAlefHandle config_handle = cberg_crawl_config_from_json("{\"respect_robots_txt\":false}");
    assert(config_handle != 0 && "failed to parse config");
    CBERGAlefHandle engine = cberg_create_engine(config_handle);
    cberg_crawl_config_free(config_handle);
    assert(engine != 0 && "failed to create engine");
    const char* mock_per_fixture = getenv("MOCK_SERVER_CONTENT_UTF8_BOM");
    const char* mock_base = getenv("MOCK_SERVER_URL");
    char url[2048];
    if (mock_per_fixture && mock_per_fixture[0] != '\0') {
        snprintf(url, sizeof(url), "%s", mock_per_fixture);
    } else {
        assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
        snprintf(url, sizeof(url), "%s/fixtures/content_utf8_bom", mock_base);
    }
    CBERGAlefHandle result = cberg_scrape(engine, url);
    assert(result != 0 && "expected call to succeed");
    char* content_detected_charset = cberg_scrape_result_detected_charset(result);
    char* html = cberg_scrape_result_html(result);
    assert(content_detected_charset != NULL && strcmp(content_detected_charset, "utf-8") == 0 && "equals assertion failed");
    assert(html != NULL && strlen(html) > 0 && strcmp(html, "[]") != 0 && strcmp(html, "{}") != 0 && "expected non-empty value");
    cberg_free_string(content_detected_charset);
    cberg_free_string(html);
    cberg_scrape_result_free(result);
    cberg_crawl_engine_handle_free(engine);
}
