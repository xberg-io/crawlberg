 ...
 * alef:hash:61f0f4ca3d2f6418f3aff9882f68aa1e51c8bd3af3183e14aca3b07dc6da1cb5
...
void test_scrape_asset_dedup(void) {
    ...
    assert(assets_length == 2 && "equals assertion failed");
    {
        ...
        for (int _wc_i = 0; _wc_i < _wc_count; _wc_i++) {
            ...
            char* _wc_val = alef_json_get_string(_wc_elem, "content_hash");
            if (_wc_val != NULL && strlen(_wc_val) > 0) { found = 1; }
            free(_wc_val);
            free(_wc_elem);
        }
        assert(found && "expected some element to have a non-empty value");
    }
...
void test_scrape_malformed_html(void) {
    ...
    assert(status_code == 200 && "equals assertion failed");
    assert(html != NULL && strlen(html) > 0 && strcmp(html, "[]") != 0 && strcmp(html, "{}") != 0 && "expected non-empty value");
    assert(metadata_title != NULL && strstr(metadata_title, "Malformed Page") != NULL && "expected to contain substring");
    assert((metadata_description == NULL || strlen(metadata_description) == 0 || strcmp(metadata_description, "[]") == 0 || strcmp(metadata_description, "{}") == 0) && "expected empty value");
    cberg_free_string(html);
    cberg_free_string(metadata_title);
    cberg_free_string(metadata_description);
    cberg_page_metadata_free(metadata_handle);
