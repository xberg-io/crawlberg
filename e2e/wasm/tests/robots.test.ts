...
describe("robots", () => {
  ...
  it("robots_allow_override: Allow directive overrides Disallow for specific paths", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_ALLOW_OVERRIDE ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_allow_override`;
    const result = await scrape(engine, url);
    expect(result.isAllowed).toBe(true);
  }, 30000);
  ...
  it("robots_disallow_path: Robots.txt disallows specific paths", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_DISALLOW_PATH ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_disallow_path`;
    const result = await scrape(engine, url);
  ...
  it("robots_meta_nofollow: Detects nofollow meta robots tag and skips link extraction", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_META_NOFOLLOW ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_meta_nofollow`;
    const result = await scrape(engine, url);
  ...
  it("robots_meta_noindex: Detects noindex meta robots tag in HTML page", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_META_NOINDEX ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_meta_noindex`;
    const result = await scrape(engine, url);
  ...
  it("robots_request_rate: Parses request-rate directive from robots.txt", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_REQUEST_RATE ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_request_rate`;
    const result = await scrape(engine, url);
  ...
  it("robots_wildcard_paths: Handles wildcard Disallow patterns in robots.txt", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_WILDCARD_PATHS ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_wildcard_paths`;
    const result = await scrape(engine, url);
    expect(result.isAllowed).toBe(false);
  }, 30000);
  it("robots_x_robots_tag: Respects X-Robots-Tag HTTP header directives", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_ROBOTS_X_ROBOTS_TAG ?? `${process.env.MOCK_SERVER_URL}/fixtures/robots_x_robots_tag`;
    const result = await scrape(engine, url);
