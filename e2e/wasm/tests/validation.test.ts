...
// alef:hash:ce902ddaadedd60d8258693c75f20c32fc081ffa6a96241715bb6e6b58439133
...
describe("validation", () => {
	...
	it("validation_ssrf_ipv4_mapped_ipv6_denied: SSRF policy rejects an IPv4 loopback address embedded in an IPv6 literal", async () => {
		await expect(async () => {
			...
			engineConfig.ssrf.denyPrivate = false;
			const engine = createEngine(engineConfig);
			const url = `${process.env.MOCK_SERVER_URL}/fixtures/validation_ssrf_ipv4_mapped_ipv6_denied`;
			await scrape(engine, url);
		}).rejects.toThrow();
	...
	it("validation_ssrf_nat64_loopback_denied: SSRF policy rejects an IPv4 loopback address embedded in the well-known NAT64 prefix", async () => {
		await expect(async () => {
			...
			engineConfig.ssrf.denyPrivate = false;
			const engine = createEngine(engineConfig);
			const url = `${process.env.MOCK_SERVER_URL}/fixtures/validation_ssrf_nat64_loopback_denied`;
			await scrape(engine, url);
		}).rejects.toThrow();
		// skipped: assertion type 'error' has no accessor for error field <none> in this backend
	}, 30000);
