...
describe("map", () => {
  ...
  it("map_exclude_patterns: Excludes URLs matching patterns from URL map", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_MAP_EXCLUDE_PATTERNS ?? `${process.env.MOCK_SERVER_URL}/fixtures/map_exclude_patterns`;
    const result = await mapUrls(engine, url);
  ...
  it("map_limit_pagination: Limits map result count to specified maximum", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_MAP_LIMIT_PAGINATION ?? `${process.env.MOCK_SERVER_URL}/fixtures/map_limit_pagination`;
    const result = await mapUrls(engine, url);
