...
// alef:hash:a9dc8eaf8a024972002b2d54825208a2ba032ebff9bd2aabebe051d1768010dc
...
describe("redirect", () => {
	...
	it("redirect_loop: Detects redirect loop (A -> B -> A) and stops following without reaching a final page", async () => {
		...
		const result = await crawl(engine, url);
    expect(Number(result.redirectCount)).toBe(1);
    expect(Number(result.pages[0].statusCode)).toBe(302);
    ...
    expect(Number(result.pages[0].statusCode)).toBe(301);
