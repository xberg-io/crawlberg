...
// alef:hash:c873e1e21d57a152dd27ea524514863b0b58d6796b7a5902293e8129dcaba1f3
...
describe("redirect", () => {
    ...
    const unresolvedAssertion = "alef resolved no assertion for fixture `redirect_loop`: 1 assertion(s) name a field path the availability oracle rejected";
	...
	it("redirect_max_exceeded: Aborts when redirect count exceeds max_redirects limit", async () => {
		...
		const result = await crawl(engine, url);
    // skipped: field 'is_error' not available on result type
    const unresolvedAssertion = "alef resolved no assertion for fixture `redirect_max_exceeded`: 1 assertion(s) name a field path the availability oracle rejected";
    expect(unresolvedAssertion).toBeNull();
