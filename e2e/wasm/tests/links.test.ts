...
describe("links", () => {
  it("links_anchor_fragment: Identifies fragment-only links as anchor type", async () => {
    ...
    const url =
      process.env.MOCK_SERVER_LINKS_ANCHOR_FRAGMENT ?? `${process.env.MOCK_SERVER_URL}/fixtures/links_anchor_fragment`;
  ...
  it("links_document_types: Detects PDF, DOCX, XLSX links as document type", async () => {
    ...
    const engine = createEngine(engineConfig);
    const url =
      process.env.MOCK_SERVER_LINKS_DOCUMENT_TYPES ?? `${process.env.MOCK_SERVER_URL}/fixtures/links_document_types`;
    const result = await scrape(engine, url);
