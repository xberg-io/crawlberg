...
// alef:hash:5c6c93a9e4877c35ec1ab232edf2342b9cb864a2f2d3ed49d53040d6f0789f95
// To regenerate: alef generate
// To verify freshness: alef verify

import { describe, expect, it } from "vitest";
import { scrape, crawl, createEngine, WasmCrawlConfig, WasmCrawlStrategyKind, WasmAuthConfig, WasmBrowserConfig, WasmContentConfig, WasmProxyConfig, WasmSsrfPolicy } from "@xberg-io/crawlberg-wasm";
...
describe("strategy", () => {
	...
	it("strategy_bfs_default_order: BFS strategy visits pages in breadth-first order", async () => {
		const engineConfig = WasmCrawlConfig.default();
		engineConfig.crawlStrategy = WasmCrawlStrategyKind.Bfs;
		engineConfig.maxConcurrent = 1;
		engineConfig.maxDepth = 2;
		engineConfig.ssrf.denyPrivate = false;
		const engine = createEngine(engineConfig);
	...
	it("strategy_dfs_depth_first: DFS strategy visits pages in depth-first order", async () => {
		const engineConfig = WasmCrawlConfig.default();
		engineConfig.crawlStrategy = WasmCrawlStrategyKind.Dfs;
		engineConfig.maxConcurrent = 1;
		engineConfig.maxDepth = 2;
		engineConfig.ssrf.denyPrivate = false;
		const engine = createEngine(engineConfig);
