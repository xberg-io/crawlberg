...
# alef:hash:b1de9621599fad5648c04229d25a8695b6322504ff4bcc123c1127c02271b27a
...
defmodule E2e.ContentTest do
  ...
  describe "content_default_preset" do
    test "content_default_preset" do
      engine_config = "{\"respect_robots_txt\":false}"
      {:ok, engine} = Crawlberg.create_engine(engine_config)
      url = (System.get_env("MOCK_SERVER_URL") || "") <> "/fixtures/content_default_preset"
      {:ok, result} = Crawlberg.scrape(engine, url)
      assert String.contains?(to_string(result.markdown.content), "Main Article Title")
      assert String.contains?(to_string(result.markdown.content), "Footer content")
      refute String.contains?(to_string(result.markdown.content), "Sidebar content")
    end
  end
  ...
  describe "content_main_only" do
    test "content_main_only" do
      ...
      {:ok, result} = Crawlberg.scrape(engine, url)
      assert String.contains?(to_string(result.markdown.content), "Main Article Title")
      refute String.contains?(to_string(result.markdown.content), "Footer content")
      refute String.contains?(to_string(result.markdown.content), "Sidebar content")
    end
  end
