...
// alef:hash:e1abdecaff8fc0285e70fdcb200a16261f956ec26e8a9a4155cade4f6636b3e9
...
namespace Crawlberg
{
    ...
    public class ContentTests
    {
        ...
        [Fact]
        public async Task Test_ContentDefaultPreset()
        {
            // Default preprocessing keeps the footer that the aggressive preset drops
            var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"respect_robots_txt\":false}", ConfigOptions)!;
            var engine = CrawlbergConverter.CreateEngine(engineConfig);
            var url = Environment.GetEnvironmentVariable("MOCK_SERVER_URL") + "/fixtures/content_default_preset";
            var result = await CrawlbergConverter.ScrapeAsync(engine, url);
            Assert.Contains("Main Article Title", result.Markdown!.Content.ToString());
            Assert.Contains("Footer content", result.Markdown!.Content.ToString());
            Assert.DoesNotContain("Sidebar content", result.Markdown!.Content.ToString());
        ...
        public async Task Test_ContentMainOnly()
        {
            ...
            var result = await CrawlbergConverter.ScrapeAsync(engine, url);
            Assert.Contains("Main Article Title", result.Markdown!.Content.ToString());
            Assert.DoesNotContain("Footer content", result.Markdown!.Content.ToString());
            Assert.DoesNotContain("Sidebar content", result.Markdown!.Content.ToString());
