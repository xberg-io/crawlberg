...
// alef:hash:686c0c6551d5b5fb7b444f2a2159c09dc17a793d595e8b0429e2c9c8159995f2
...
namespace Crawlberg
{
    ...
    public class CrawlTests
    {
        ...
        [Fact]
        public async Task Test_CrawlMaxLinksPerPage()
        {
            ...
            var engine = CrawlbergConverter.CreateEngine(engineConfig);
            var _pfUrl_url = Environment.GetEnvironmentVariable("MOCK_SERVER_CRAWL_MAX_LINKS_PER_PAGE");
            var url = !string.IsNullOrEmpty(_pfUrl_url) ? _pfUrl_url : Environment.GetEnvironmentVariable("MOCK_SERVER_URL") + "/fixtures/crawl_max_links_per_page";
            var result = await CrawlbergConverter.CrawlAsync(engine, url);
            Assert.True(result.Pages.Count <= 3, "expected <= 3");
