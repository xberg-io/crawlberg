...
// alef:hash:eb900b1ebbe97e62b451989a217bb6d81ace8737ff9ba7e2891c9ec7f8226f8a
...
namespace Crawlberg
{
    ...
    public class RobotsTests
    {
        ...
        [Fact]
        public async Task Test_RobotsCrawlDisallowBlocksChild()
        {
            ...
            var engine = CrawlbergConverter.CreateEngine(engineConfig);
            var _pfUrl_url = Environment.GetEnvironmentVariable("MOCK_SERVER_ROBOTS_CRAWL_DISALLOW_BLOCKS_CHILD");
            var url = !string.IsNullOrEmpty(_pfUrl_url) ? _pfUrl_url : Environment.GetEnvironmentVariable("MOCK_SERVER_URL") + "/fixtures/robots_crawl_disallow_blocks_child";
        ...
        [Fact]
        public async Task Test_RobotsCrawlUnreachable5xx()
        {
            // Crawl stops before any fetch when robots.txt is unreachable
            var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"max_depth\":1,\"respect_robots_txt\":true}", ConfigOptions)!;
            var engine = CrawlbergConverter.CreateEngine(engineConfig);
            var _pfUrl_url = Environment.GetEnvironmentVariable("MOCK_SERVER_ROBOTS_CRAWL_UNREACHABLE_5XX");
            var url = !string.IsNullOrEmpty(_pfUrl_url) ? _pfUrl_url : Environment.GetEnvironmentVariable("MOCK_SERVER_URL") + "/fixtures/robots_crawl_unreachable_5xx";
            var result = await CrawlbergConverter.CrawlAsync(engine, url);
            Assert.True(result.Pages.Count == 0);
            Assert.True(result.WasSkipped);
            Assert.Contains("robots_unreachable", result.Error.ToString());
        ...
        [Fact]
        public async Task Test_RobotsUnreachable5xx()
        {
            // Unreachable robots.txt (503) reports is_allowed false
            var engineConfig = JsonSerializer.Deserialize<CrawlConfig>("{\"respect_robots_txt\":true}", ConfigOptions)!;
            var engine = CrawlbergConverter.CreateEngine(engineConfig);
            var _pfUrl_url = Environment.GetEnvironmentVariable("MOCK_SERVER_ROBOTS_UNREACHABLE_5XX");
            var url = !string.IsNullOrEmpty(_pfUrl_url) ? _pfUrl_url : Environment.GetEnvironmentVariable("MOCK_SERVER_URL") + "/fixtures/robots_unreachable_5xx";
            var result = await CrawlbergConverter.ScrapeAsync(engine, url);
            Assert.False(result.IsAllowed);
        ...
        public async Task Test_RobotsXRobotsTag()
        {
            ...
            var result = await CrawlbergConverter.ScrapeAsync(engine, url);
            Assert.Equal("noindex, nofollow", result.XRobotsTag);
            Assert.True(result.NoindexDetected);
            Assert.True(result.NofollowDetected);
