<?php

// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:495dedc1a12a77710a880dd65741be866b67ab4db2f2026df3e8591ba7fc9a69
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code

declare(strict_types=1);

namespace Crawlberg\E2e;

use Crawlberg\Crawlberg;
use Crawlberg\CrawlConfig;
use PHPUnit\Framework\TestCase;

/** E2e tests for category: stealth. */
final class StealthTest extends TestCase
{
    /** User-agent rotation config is accepted and crawl succeeds */
    public function test_stealth_ua_rotation_config(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['userAgents' => [
            'Mozilla/5.0 (Windows NT 10.0)',
            'Chrome/120.0.0.0',
        ]]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_URL') . '/fixtures/stealth_ua_rotation_config';
        $result = Crawlberg::scrape($engine, $url);

        $this->assertEquals(200, $result->statusCode);
    }

    /** User-agent rotation cycles through multiple agents across multiple requests */
    public function test_stealth_ua_rotation_round_robin(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode([
            'maxDepth' => 1,
            'maxPages' => 3,
            'userAgents' => [
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) TestAgent-1',
                'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15) TestAgent-2',
            ],
        ]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_STEALTH_UA_ROTATION_ROUND_ROBIN')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/stealth_ua_rotation_round_robin';
        $result = Crawlberg::crawl($engine, $url);

        $this->assertGreaterThanOrEqual(2, count($result->getPages()));
    }

    /** Custom user-agent string is applied for single domain crawl */
    public function test_stealth_ua_rotation_single_domain(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode([
            'maxDepth' => 0,
            'stayOnDomain' => true,
            'userAgents' => ['Mozilla/5.0 TestBot/1.0 (+http://example.com/bot)'],
        ]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_URL') . '/fixtures/stealth_ua_rotation_single_domain';
        $result = Crawlberg::crawl($engine, $url);

        $this->assertEquals(200, $result->getPages()[0]->statusCode);
        $this->assertEquals(1, count($result->getPages()));
    }
}
