...
// alef:hash:b9803c98940b1a337be4ee7c649ea0711390d72b374376630f706fe3b0cf2698
...
final class ContentTest extends TestCase
{
    ...
    public function test_content_charset_iso8859(): void
    {
        ...
        $this->assertEquals("iso-8859-1", ($result->detectedCharset ?? null));
    ...
    public function test_content_default_preset(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(["respectRobotsTxt" => false]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_URL') . '/fixtures/content_default_preset';
        $result = Crawlberg::scrape($engine, $url);

        $this->assertStringContainsString("Main Article Title", $result->getMarkdown()?->content);
        $this->assertStringContainsString("Footer content", $result->getMarkdown()?->content);
        $this->assertStringNotContainsString("Sidebar content", $result->getMarkdown()?->content);
    ...
    public function test_content_main_only(): void
    {
        ...
        $this->assertStringContainsString("Main Article Title", $result->getMarkdown()?->content);
        $this->assertStringNotContainsString("Footer content", $result->getMarkdown()?->content);
        $this->assertStringNotContainsString("Sidebar content", $result->getMarkdown()?->content);
    ...
    public function test_content_non_ascii_body_limit(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(["maxBodySize" => 1, "respectRobotsTxt" => false]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_URL') . '/fixtures/content_non_ascii_body_limit';
        $result = Crawlberg::scrape($engine, $url);

        $this->assertEquals(200, $result->statusCode);
        $this->assertEquals(0, $result->bodySize);
    ...
    public function test_content_utf8_bom(): void
    {
        ...
        $this->assertEquals("utf-8", ($result->detectedCharset ?? null));
        $this->assertNotSame('', $result->html ?? '', 'expected non-empty value');
