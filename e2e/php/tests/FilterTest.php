<?php

// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:17f9f8de28107eff953439f136be0939f7d878b9c0bfeba5966d724bf5eb3056
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code

declare(strict_types=1);

namespace Crawlberg\E2e;

use Crawlberg\Crawlberg;
use Crawlberg\CrawlConfig;
use PHPUnit\Framework\TestCase;

/** E2e tests for category: filter. */
final class FilterTest extends TestCase
{
    /** BM25 filter works during multi-page crawl, keeping relevant pages */
    public function test_filter_bm25_crawl_integration(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxConcurrent' => 1, 'maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_BM25_CRAWL_INTEGRATION')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_bm25_crawl_integration';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::crawl($engine, $url);

        // skipped: field 'filter.remaining_contain_keyword' not available on result type
    }

    /** BM25 filter with empty query passes all pages through */
    public function test_filter_bm25_empty_query(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_BM25_EMPTY_QUERY')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_bm25_empty_query';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::crawl($engine, $url);

        // skipped: field 'crawl.pages_crawled' not available on result type
    }

    /** BM25 filter with very high threshold filters out all pages */
    public function test_filter_bm25_high_threshold(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_BM25_HIGH_THRESHOLD')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_bm25_high_threshold';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::scrape($engine, $url);

        // skipped: field 'filter.pages_after_filter' not available on result type
    }

    /** BM25 filter keeps only pages relevant to the query */
    public function test_filter_bm25_relevant_pages(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_BM25_RELEVANT_PAGES')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_bm25_relevant_pages';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::scrape($engine, $url);

        // skipped: field 'filter.remaining_contain_keyword' not available on result type
    }

    /** BM25 filter with zero threshold passes all pages */
    public function test_filter_bm25_threshold_zero(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_BM25_THRESHOLD_ZERO')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_bm25_threshold_zero';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::crawl($engine, $url);

        // skipped: field 'crawl.pages_crawled' not available on result type
    }

    /** NoopFilter keeps all pages during a multi-page crawl */
    public function test_filter_noop_crawl_all_kept(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxConcurrent' => 1, 'maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_NOOP_CRAWL_ALL_KEPT')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_noop_crawl_all_kept';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::crawl($engine, $url);

        // skipped: field 'filter.pages_after_filter' not available on result type
    }

    /** No content filter passes all crawled pages through */
    public function test_filter_noop_passes_all(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_FILTER_NOOP_PASSES_ALL')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/filter_noop_passes_all';
        $this->expectNotToPerformAssertions();
        $result = Crawlberg::crawl($engine, $url);

        // skipped: field 'crawl.pages_crawled' not available on result type
    }
}
