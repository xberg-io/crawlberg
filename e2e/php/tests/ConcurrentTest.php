...
final class ConcurrentTest extends TestCase
{
    ...
    public function test_concurrent_depth_two_fan_out(): void
    {
        ...
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_CONCURRENT_DEPTH_TWO_FAN_OUT')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/concurrent_depth_two_fan_out';
        $result = Crawlberg::crawl($engine, $url);
    ...
    public function test_concurrent_max_pages_exact(): void
    {
        ...
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_CONCURRENT_MAX_PAGES_EXACT')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/concurrent_max_pages_exact';
        $result = Crawlberg::crawl($engine, $url);
    ...
    public function test_concurrent_partial_errors(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['maxConcurrent' => 3, 'maxDepth' => 1]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_CONCURRENT_PARTIAL_ERRORS')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/concurrent_partial_errors';
        $result = Crawlberg::crawl($engine, $url);
    ...
    public function test_concurrent_respects_max_pages(): void
    {
        ...
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_CONCURRENT_RESPECTS_MAX_PAGES')
        ?: getenv('MOCK_SERVER_URL') . '/fixtures/concurrent_respects_max_pages';
        $result = Crawlberg::crawl($engine, $url);
