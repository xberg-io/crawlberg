...
// alef:hash:82e5c44632dd52d5b29e487c8357e38e46de964150b37176a8005f77ecb185bf
...
final class RobotsTest extends TestCase
{
    ...
    public function test_robots_crawl_disallow_blocks_child(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(["maxDepth" => 1, "respectRobotsTxt" => true]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_ROBOTS_CRAWL_DISALLOW_BLOCKS_CHILD') ?: getenv('MOCK_SERVER_URL') . '/fixtures/robots_crawl_disallow_blocks_child';
    ...
    public function test_robots_crawl_unreachable_5xx(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(["maxDepth" => 1, "respectRobotsTxt" => true]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_ROBOTS_CRAWL_UNREACHABLE_5XX') ?: getenv('MOCK_SERVER_URL') . '/fixtures/robots_crawl_unreachable_5xx';
        $result = Crawlberg::crawl($engine, $url);

        $this->assertEquals(0, count($result->getPages()));
        $this->assertEquals(true, $result->wasSkipped);
        $this->assertStringContainsString("robots_unreachable", ($result->error ?? null));
    ...
    public function test_robots_unreachable_5xx(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(["respectRobotsTxt" => true]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_ROBOTS_UNREACHABLE_5XX') ?: getenv('MOCK_SERVER_URL') . '/fixtures/robots_unreachable_5xx';
        $result = Crawlberg::scrape($engine, $url);
    ...
    public function test_robots_x_robots_tag(): void
    {
        ...
        $this->assertEquals("noindex, nofollow", ($result->xRobotsTag ?? null));
        $this->assertEquals(true, $result->noindexDetected);
        $this->assertEquals(true, $result->nofollowDetected);
