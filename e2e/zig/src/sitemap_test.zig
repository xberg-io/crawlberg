...
test "sitemap_basic" {
    ...
    try testing.expectEqual(@as(usize, 4), result.object.get("urls").?.array.items.len);
}
...
test "sitemap_compressed_gzip" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqual(@as(usize, 3), result.object.get("urls").?.array.items.len);
}
...
test "sitemap_empty" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqual(@as(usize, 0), result.object.get("urls").?.array.items.len);
}
...
test "sitemap_xhtml_links" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqual(@as(usize, 2), result.object.get("urls").?.array.items.len);
}
