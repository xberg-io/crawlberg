...
// alef:hash:2944f7d58efeb4bdeb29051d2aa17fbb79524ff7f8091d23d33b19408fc408ea
...
test "robots_crawl_delay" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqual(@as(i64, 2), result.object.get("crawl_delay").?.integer);
}

test "robots_crawl_disallow_blocks_child" {
    ...
    const url = if (std.c.getenv("MOCK_SERVER_ROBOTS_CRAWL_DISALLOW_BLOCKS_CHILD")) |_pf| try std.fmt.allocPrint(allocator, "{s}", .{std.mem.span(_pf)}) else try std.fmt.allocPrint(allocator, "{s}/fixtures/robots_crawl_disallow_blocks_child", .{if (std.c.getenv("MOCK_SERVER_URL")) |v| std.mem.span(v) else "http://localhost:8080"});
    defer allocator.free(url);
    const _result_json = try crawlberg.crawl("{\"max_depth\":1,\"respect_robots_txt\":true}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "robots_crawl_unreachable_5xx" {
    ...
    const url = if (std.c.getenv("MOCK_SERVER_ROBOTS_CRAWL_UNREACHABLE_5XX")) |_pf| try std.fmt.allocPrint(allocator, "{s}", .{std.mem.span(_pf)}) else try std.fmt.allocPrint(allocator, "{s}/fixtures/robots_crawl_unreachable_5xx", .{if (std.c.getenv("MOCK_SERVER_URL")) |v| std.mem.span(v) else "http://localhost:8080"});
    defer allocator.free(url);
    const _result_json = try crawlberg.crawl("{\"max_depth\":1,\"respect_robots_txt\":true}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    ...
    try testing.expectEqual(true, result.object.get("was_skipped").?.bool);
    {
        ...
        try testing.expect(std.mem.indexOf(u8, _js, "robots_unreachable") != null);
    }
}
...
test "robots_request_rate" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqual(@as(i64, 5), result.object.get("crawl_delay").?.integer);
    try testing.expectEqual(true, result.object.get("is_allowed").?.bool);
}
...
test "robots_unreachable_5xx" {
    ...
    const url = if (std.c.getenv("MOCK_SERVER_ROBOTS_UNREACHABLE_5XX")) |_pf| try std.fmt.allocPrint(allocator, "{s}", .{std.mem.span(_pf)}) else try std.fmt.allocPrint(allocator, "{s}/fixtures/robots_unreachable_5xx", .{if (std.c.getenv("MOCK_SERVER_URL")) |v| std.mem.span(v) else "http://localhost:8080"});
    defer allocator.free(url);
    const _result_json = try crawlberg.scrape("{\"respect_robots_txt\":true}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
...
test "robots_x_robots_tag" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqualStrings("noindex, nofollow", result.object.get("x_robots_tag").?.string);
    try testing.expectEqual(true, result.object.get("noindex_detected").?.bool);
    try testing.expectEqual(true, result.object.get("nofollow_detected").?.bool);
}
