...
// alef:hash:c00d0e03d252ae1eb894b9bfbbff9d45e79991e705998cc057708746a1a93a85
...
test "filter_bm25_crawl_integration" {
    ...
    const _result_json = try crawlberg.crawl("{\"bm25_query\":\"rust programming\",\"bm25_threshold\":0.1,\"content_filter\":\"bm25\",\"max_concurrent\":1,\"max_depth\":1}", url);
    ...
    const result = &_parsed.value;
    {
        const _WildcardCheck = struct {
            fn check(_wce: std.json.Value) anyerror!void {
                {
                    const _jv = (_wce.object.get("html") orelse return error.WildcardElementFieldMissing);
        ...
        var _wc_found = false;
        for (result.object.get("pages").?.array.items) |_wc_item| {
            _WildcardCheck.check(_wc_item) catch continue;
            _wc_found = true;
            break;
        }
        try testing.expect(_wc_found); // no element of 'pages' matched
    }
}
...
test "filter_bm25_empty_query" {
    ...
    defer allocator.free(url);
    const _result_json = try crawlberg.crawl("{\"bm25_query\":\"\",\"bm25_threshold\":0.1,\"content_filter\":\"bm25\",\"max_depth\":1}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "filter_bm25_high_threshold" {
    ...
    defer allocator.free(url);
    const _result_json = try crawlberg.crawl("{\"bm25_query\":\"quantum physics\",\"bm25_threshold\":100.0,\"content_filter\":\"bm25\",\"max_depth\":1}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "filter_bm25_relevant_pages" {
    ...
    defer allocator.free(url);
    const _result_json = try crawlberg.crawl("{\"bm25_query\":\"rust programming language\",\"bm25_threshold\":0.1,\"content_filter\":\"bm25\",\"max_depth\":1}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
...
test "filter_bm25_threshold_zero" {
    ...
    defer allocator.free(url);
    const _result_json = try crawlberg.crawl("{\"bm25_query\":\"unrelated query\",\"bm25_threshold\":0.0,\"content_filter\":\"bm25\",\"max_depth\":1}", url);
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
