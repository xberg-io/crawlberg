...
// alef:hash:3a0521aee4a206344b9ae5018af1e83179a5a477ec83dbd0a07ae84101cfce85
...
func Test_ContentCharsetIso8859(t *testing.T) {
	...
	if result.DetectedCharset != nil {
		detectedCharset = string(*result.DetectedCharset)
	}
	if string(detectedCharset) != `iso-8859-1` {
		t.Errorf("equals mismatch: got %v", detectedCharset)
	}
}

func Test_ContentDefaultPreset(t *testing.T) {
	...
	url := os.Getenv("MOCK_SERVER_URL") + "/fixtures/content_default_preset"
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if !strings.Contains(string(result.Markdown.Content), `Main Article Title`) {
		t.Errorf("expected to contain %s, got %v", `Main Article Title`, result.Markdown.Content)
	}
	if !strings.Contains(string(result.Markdown.Content), `Footer content`) {
		t.Errorf("expected to contain %s, got %v", `Footer content`, result.Markdown.Content)
	}
	if strings.Contains(string(result.Markdown.Content), `Sidebar content`) {
		t.Errorf("expected NOT to contain %s, got %v", `Sidebar content`, result.Markdown.Content)
	}
}
...
func Test_ContentMainOnly(t *testing.T) {
	...
	if strings.Contains(string(result.Markdown.Content), `Footer content`) {
		t.Errorf("expected NOT to contain %s, got %v", `Footer content`, result.Markdown.Content)
	}
	if strings.Contains(string(result.Markdown.Content), `Sidebar content`) {
		t.Errorf("expected NOT to contain %s, got %v", `Sidebar content`, result.Markdown.Content)
	}
...
func Test_ContentNonAsciiBodyLimit(t *testing.T) {
	...
	url := os.Getenv("MOCK_SERVER_URL") + "/fixtures/content_non_ascii_body_limit"
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	...
	if result.BodySize != 0 {
		t.Errorf("equals mismatch: got %v", result.BodySize)
	}
}
...
func Test_ContentUtf8Bom(t *testing.T) {
	...
	if string(detectedCharset) != `utf-8` {
		t.Errorf("equals mismatch: got %v", detectedCharset)
	}
	if len(result.HTML) == 0 {
		t.Errorf("expected non-empty value")
