// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:e05e0b73459659ca37fff157509d24730ddf92421c12083b3515e4b35b4a0364
// To regenerate: alef generate
// To verify freshness: alef verify

// E2e tests for category: robots
package e2e_test

import (
	"encoding/json"
	"os"
	"strings"
	"testing"

	pkg "github.com/xberg-io/crawlberg/packages/go"
)

func Test_RobotsAllowAll(t *testing.T) {
	// Permissive robots.txt allows all paths
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_ALLOW_ALL")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_allow_all"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsAllowOverride(t *testing.T) {
	// Allow directive overrides Disallow for specific paths
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_ALLOW_OVERRIDE")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_allow_override"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsCommentsHandling(t *testing.T) {
	// Correctly parses robots.txt with inline and line comments
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true,"user_agent":"crawlberg"}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_COMMENTS_HANDLING")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_comments_handling"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsCrawlDelay(t *testing.T) {
	// Respects crawl-delay directive from robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true,"user_agent":"crawlberg"}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_CRAWL_DELAY")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_crawl_delay"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.CrawlDelay == nil || *result.CrawlDelay != 2 {
		t.Errorf("equals mismatch: got %v", result.CrawlDelay)
	}
}

func Test_RobotsCrawlDisallowBlocksChild(t *testing.T) {
	// Crawl skips child URLs disallowed by robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"max_depth":1,"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_CRAWL_DISALLOW_BLOCKS_CHILD")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_crawl_disallow_blocks_child"
	}
	result, err := pkg.Crawl(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if len(result.Pages) != 2 {
		t.Errorf("equals mismatch: got %v", len(result.Pages))
	}
}

func Test_RobotsCrawlUnreachable5xx(t *testing.T) {
	// Crawl stops before any fetch when robots.txt is unreachable
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"max_depth":1,"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_CRAWL_UNREACHABLE_5XX")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_crawl_unreachable_5xx"
	}
	result, err := pkg.Crawl(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	var error string
	if result.Error != nil {
		error = string(*result.Error)
	}
	if len(result.Pages) != 0 {
		t.Errorf("equals mismatch: got %v", len(result.Pages))
	}
	if result.WasSkipped != true {
		t.Errorf("equals mismatch: got %v", result.WasSkipped)
	}
	if !strings.Contains(string(error), `robots_unreachable`) {
		t.Errorf("expected to contain %s, got %v", `robots_unreachable`, error)
	}
}

func Test_RobotsDisallowPath(t *testing.T) {
	// Robots.txt disallows specific paths
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_DISALLOW_PATH")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_disallow_path"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != false {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsMetaNofollow(t *testing.T) {
	// Detects nofollow meta robots tag and skips link extraction
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_META_NOFOLLOW")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_meta_nofollow"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.NofollowDetected != true {
		t.Errorf("equals mismatch: got %v", result.NofollowDetected)
	}
}

func Test_RobotsMetaNoindex(t *testing.T) {
	// Detects noindex meta robots tag in HTML page
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_META_NOINDEX")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_meta_noindex"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.NoindexDetected != true {
		t.Errorf("equals mismatch: got %v", result.NoindexDetected)
	}
}

func Test_RobotsMissing404(t *testing.T) {
	// Missing robots.txt (404) allows all crawling
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_MISSING_404")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_missing_404"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsMultipleUserAgents(t *testing.T) {
	// Picks the most specific user-agent block from robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true,"user_agent":"SpecificBot"}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_MULTIPLE_USER_AGENTS")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_multiple_user_agents"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsRequestRate(t *testing.T) {
	// Parses request-rate directive from robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true,"user_agent":"crawlberg"}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_REQUEST_RATE")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_request_rate"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.CrawlDelay == nil || *result.CrawlDelay != 5 {
		t.Errorf("equals mismatch: got %v", result.CrawlDelay)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsSitemapDirective(t *testing.T) {
	// Discovers sitemap URL from Sitemap directive in robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_SITEMAP_DIRECTIVE")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_sitemap_directive"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != true {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsUnreachable5xx(t *testing.T) {
	// Unreachable robots.txt (503) reports is_allowed false
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_UNREACHABLE_5XX")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_unreachable_5xx"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != false {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsUserAgentSpecific(t *testing.T) {
	// Matches user-agent specific rules in robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true,"user_agent":"CrawlbergBot"}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_USER_AGENT_SPECIFIC")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_user_agent_specific"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != false {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsWildcardPaths(t *testing.T) {
	// Handles wildcard Disallow patterns in robots.txt
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_WILDCARD_PATHS")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_wildcard_paths"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	if result.IsAllowed != false {
		t.Errorf("equals mismatch: got %v", result.IsAllowed)
	}
}

func Test_RobotsXRobotsTag(t *testing.T) {
	// Respects X-Robots-Tag HTTP header directives
	var engineConfig pkg.CrawlConfig
	if err := json.Unmarshal([]byte(`{"respect_robots_txt":true}`), &engineConfig); err != nil {
		t.Fatalf("config parse failed: %v", err)
	}
	engine, createErr := pkg.CreateEngine(&engineConfig)
	if createErr != nil {
		t.Fatalf("create handle failed: %v", createErr)
	}
	url := os.Getenv("MOCK_SERVER_ROBOTS_X_ROBOTS_TAG")
	if url == "" {
		url = os.Getenv("MOCK_SERVER_URL") + "/fixtures/robots_x_robots_tag"
	}
	result, err := pkg.Scrape(engine, url)
	if err != nil {
		t.Fatalf("call failed: %v", err)
	}
	var xRobotsTag string
	if result.XRobotsTag != nil {
		xRobotsTag = string(*result.XRobotsTag)
	}
	if string(xRobotsTag) != `noindex, nofollow` {
		t.Errorf("equals mismatch: got %v", xRobotsTag)
	}
	if result.NoindexDetected != true {
		t.Errorf("equals mismatch: got %v", result.NoindexDetected)
	}
	if result.NofollowDetected != true {
		t.Errorf("equals mismatch: got %v", result.NofollowDetected)
	}
}
