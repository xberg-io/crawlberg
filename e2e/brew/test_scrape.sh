...
# alef:hash:05c46b36883e5b7ce53d2a3745e54029c4f24accde5668e64fbfb4a71efa7f7e
...
test_scrape_basic_html_page() {
  ...
  local val_og_title
  val_og_title=$(echo "$output" | jq -r '.metadata.og_title // empty')
...
test_scrape_malformed_html() {
  ...
  local val_metadata_title
  val_metadata_title=$(echo "$output" | jq -r '.metadata.title')
  assert_contains "$val_metadata_title" 'Malformed Page' 'metadata.title'
  local val_metadata_description
  val_metadata_description=$(echo "$output" | jq -r '.metadata.description // empty')
  assert_is_empty "$val_metadata_description" 'metadata.description'
}
