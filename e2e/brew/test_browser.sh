...
# alef:hash:fdfeeb13e10a7cdf29fe52c5797602c33b8cc9d6c5063f4396181638876e5906
...
test_browser_config_auto_no_feature() {
  ...
  assert_equals "$val_js_render_hint" 'true' 'js_render_hint'
  local val_browser_used
  val_browser_used=$(echo "$output" | jq -r '.browser_used')
  assert_equals "$val_browser_used" 'false' 'browser_used'
}
...
test_browser_crawl_mode_always() {
  ...
  local val_browser_used
  val_browser_used=$(echo "$output" | jq -r '.browser_used')
  assert_equals "$val_browser_used" 'true' 'browser_used'
}
...
test_browser_detect_minimal_page() {
  ...
  local val_js_render_hint
  val_js_render_hint=$(echo "$output" | jq -r '.js_render_hint')
  assert_equals "$val_js_render_hint" 'false' 'js_render_hint'
  local val_browser_used
  val_browser_used=$(echo "$output" | jq -r '.browser_used')
  assert_equals "$val_browser_used" 'false' 'browser_used'
}
