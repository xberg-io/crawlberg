...
# alef:hash:ccd4507e0bffcc2ba85b740d360faacf1e818c4329ffbf475a419bdec8b1ba2c
...
test_crawl_max_links_per_page() {
  ...
  local output
  output=$(crawlberg crawl "${MOCK_SERVER_CRAWL_MAX_LINKS_PER_PAGE:-${MOCK_SERVER_URL}/fixtures/crawl_max_links_per_page}" --config '{"max_links_per_page":2,"max_pages":10,"respect_robots_txt":false}' --format json --browser-mode never)
...
test_crawl_multiple_redirects_in_traversal() {
  ...
  local output
  output=$(crawlberg crawl "${MOCK_SERVER_CRAWL_MULTIPLE_REDIRECTS_IN_TRAVERSAL:-${MOCK_SERVER_URL}/fixtures/crawl_multiple_redirects_in_traversal}" --config '{"max_concurrent":1,"max_depth":2}' --format json --browser-mode never)
...
test_crawl_redirect_in_traversal() {
  ...
  local output
  output=$(crawlberg crawl "${MOCK_SERVER_CRAWL_REDIRECT_IN_TRAVERSAL:-${MOCK_SERVER_URL}/fixtures/crawl_redirect_in_traversal}" --config '{"max_concurrent":1,"max_depth":2}' --format json --browser-mode never)
...
test_crawl_subdomain_inclusion() {
  ...
  local output
  output=$(crawlberg crawl "${MOCK_SERVER_CRAWL_SUBDOMAIN_INCLUSION:-${MOCK_SERVER_URL}/fixtures/crawl_subdomain_inclusion}" --config '{"allow_subdomains":true,"max_depth":1,"respect_robots_txt":false,"stay_on_domain":true}' --format json --browser-mode never)
...
run_tests_crawl() {
  ...
  run_test test_crawl_max_links_per_page
