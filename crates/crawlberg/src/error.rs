...
impl CrawlError {
    /// Build a [`CrawlError::WafBlocked`] with no underlying error.
    ///
    /// ~keep Hand-written rather than generated by `message_constructors!`: the variant carries
    /// `vendor` beside `message`, which the macro's `(message)` and `(message, source)` shapes
    /// cannot express. The pair exists so a caller that holds the response status can attach it
    /// (crawlberg#133) without every call site restating `source: None`.
    pub fn waf_blocked(vendor: impl Into<String>, message: impl Into<String>) -> Self {
        Self::WafBlocked {
            vendor: vendor.into(),
    ...
    /// Build a [`CrawlError::WafBlocked`] preserving `source` in the error chain.
    pub fn waf_blocked_with_source(
