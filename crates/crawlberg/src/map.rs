...
async fn urls_from_direct_response(
    ...
    if is_html_content(&resp.content_type, &resp.body) {
        ...
        if let Ok(doc) = tl::parse(&parsed_html.text, ParserOptions::default()) {
            let link_base = effective_base_url(parsed_html.base_href.as_deref(), parsed_url);
            return links_as_sitemap_urls(&doc, &link_base);
        }
    }
...
fn links_as_sitemap_urls(doc: &tl::VDom<'_>, link_base: &Url) -> Vec<SitemapUrl> {
    let links = extract_links(doc, link_base);
...
mod tests {
    ...
    #[tokio::test]
    async fn map_resolves_html_links_against_the_base_an_html_parser_reads() {
        let mock = MockServer::start().await;
        let base = mock.uri();
        mount_body(
            &mock,
            "/",
            "text/html",
            r#"<html><head><xmp><base href="https://hijacked.example/"></xmp><base href="/docs/"></head>
        ...
        assert_eq!(
            urls,
            vec![format!("{base}/docs/guide")],
            "links resolve against the first real `<base href>`, not one inside xmp text"
        );
    }
