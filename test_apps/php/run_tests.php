...
// alef:hash:812fc90e3cfa9bd945eb1182164caf8ccbf8ab630338866666126195097674f4
...
$repoRoot = __DIR__ . '/../../';
$localExtCandidates = [
    $repoRoot . 'target/release/libcrawlberg_php' . $extSuffix,
    $repoRoot . 'crates/crawlberg-php/target/release/libcrawlberg_php' . $extSuffix,
];
...
if (!$pieInstalledExtPath && $resolvePieFromExtensionDir) {
    ...
    if ($registryExtDir !== '') {
        $pieInstalledExtPath = $registryExtDir . '/crawlberg.so';
    }
}
...
if (!file_exists($extPath)) {
    fwrite(STDERR, "error: no crawlberg PHP extension build found.\n");
    foreach ($localExtCandidates as $candidate) {
    ...
    fwrite(STDERR, "  cargo build --release -p crawlberg-php\n");
    exit(1);
}
...
$php = PHP_BINARY;
$isolatedIni = alef_build_isolated_ini($php, 'crawlberg');
...
$process = proc_open($preflightCmd, $descriptorSpec, $pipes, null, ['ALEF_PREFLIGHT_EXT_NAME' => 'crawlberg'] + $_ENV, [
    'bypass_shell' => true,
]);
if ($process === false) {
    fwrite(STDERR, "error: failed to spawn PHP preflight check for the crawlberg extension.\n");
    exit(1);
}
...
if (stripos($preflightStderr, 'already loaded') !== false) {
    fwrite(STDERR, "error: the crawlberg extension is already loaded from an ambient php.ini\n");
...
if ($preflightExit !== 0 || $preflightStdout === '') {
    fwrite(STDERR, "error: the crawlberg extension did not load from $extPath.\n");
...
if (version_compare($loadedVersion, '1.7.1', '<')) {
    fwrite(STDERR, "error: loaded crawlberg extension version mismatch.\n");
    fwrite(STDERR, "  extension at $extPath reports version: $loadedVersion\n");
    fwrite(STDERR, "  expected version: >=1.7.1\n");
