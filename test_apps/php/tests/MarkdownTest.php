...
// alef:hash:aedf2e5b85443f56fc00d43dff86bc1341da1e880e0f82a1bcd92cd643373268
...
final class MarkdownTest extends TestCase
{
    ...
    public function test_markdown_no_frontmatter(): void
    {
        $engine_config = CrawlConfig::from_json(json_encode(['content' => ['extractMetadata' => false]]));
        $engine = Crawlberg::createEngine($engine_config);
        $url = getenv('MOCK_SERVER_URL') . '/fixtures/markdown_no_frontmatter';
        $result = Crawlberg::scrape($engine, $url);

        $this->assertEquals(200, $result->statusCode);
        $this->assertEquals('Test', $result->getMetadata()->title ?? null);
        $this->assertStringNotContainsString('---', $result->getMarkdown()?->content);
        $this->assertStringContainsString('Hello World', $result->getMarkdown()?->content);
    }
