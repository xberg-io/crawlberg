...
# alef:hash:d4ed3244ab9ea5ceaf05fb4784ff648176add3e58769ee17b24cdd01e143bb91
...
RSpec.describe "markdown" do
  ...
  it "citations_balanced_parens: Citations correctly handle links inside parentheses with balanced rendering" do
    ...
    expect(
      result.markdown.content.respond_to?(:empty?) ? !result.markdown.content.empty? : !result.markdown.content.nil?
    )
      .to(be(true))
  ...
  it "markdown_crawl_all_pages: All crawled pages have markdown field populated" do
    ...
    url = ENV.fetch("MOCK_SERVER_MARKDOWN_CRAWL_ALL_PAGES", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/markdown_crawl_all_pages"
  ...
  it "markdown_fit_content: Fit markdown removes navigation and boilerplate content" do
    engine = Crawlberg.create_engine(nil)
    url = ENV.fetch("MOCK_SERVER_MARKDOWN_FIT_CONTENT", nil) ||
      "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/markdown_fit_content"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(
      result.markdown.content.respond_to?(:empty?) ? !result.markdown.content.empty? : !result.markdown.content.nil?
  ...
  it "markdown_no_frontmatter: extract_metadata: false omits the YAML frontmatter block from markdown output" do
    engine_config = {"content" => {"extract_metadata" => false}}
    engine = Crawlberg.create_engine(engine_config.to_json)
    url = "#{ENV.fetch("MOCK_SERVER_URL")}/fixtures/markdown_no_frontmatter"
    result = Crawlberg.scrape(engine, url)
    expect(result.status_code).to(eq(200))
    expect(result.metadata.title.to_s).to(eq("Test"))
    expect(result.markdown.content.to_s).not_to(include("---"))
    expect(result.markdown.content.to_s).to(include("Hello World"))
  end
