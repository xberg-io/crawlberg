...
# alef:hash:95d58e171e4f6fa3758791f0ab552028e4752f574c6cc333e89ba7acd14f07f1
...
defmodule E2e.MarkdownTest do
  ...
  describe "citations_balanced_parens" do
    test "citations_balanced_parens" do
      ...
      assert result.markdown.content not in [nil, "", [], %{}]
  ...
  describe "markdown_no_frontmatter" do
    test "markdown_no_frontmatter" do
      engine_config = "{\"content\":{\"extract_metadata\":false}}"
      {:ok, engine} = Crawlberg.create_engine(engine_config)
      url = (System.get_env("MOCK_SERVER_URL") || "") <> "/fixtures/markdown_no_frontmatter"
      {:ok, result} = Crawlberg.scrape(engine, url)
      assert result.status_code == 200
      assert result.metadata.title == "Test"
      refute String.contains?(to_string(result.markdown.content), "---")
      assert String.contains?(to_string(result.markdown.content), "Hello World")
    end
  end
