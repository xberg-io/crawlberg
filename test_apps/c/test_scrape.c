 ...
 * alef:hash:28dfc6505140020e1adf494f53638769d013997ad5fd9bcde398400741a83817
...
void test_scrape_asset_dedup(void) {
  ...
  CBERGAlefHandle config_handle =
      cberg_crawl_config_from_json("{\"download_assets\":true}");
  ...
  assert(assets_length == 2 && "equals assertion failed");
  {
    ...
    for (int _wc_i = 0; _wc_i < _wc_count; _wc_i++) {
      ...
      if (_wc_val != NULL && strlen(_wc_val) > 0) {
        found = 1;
      }
      free(_wc_val);
      free(_wc_elem);
...
void test_scrape_asset_max_size(void) {
  ...
  CBERGAlefHandle config_handle = cberg_crawl_config_from_json(
      "{\"download_assets\":true,\"max_asset_size\":150}");
  assert(config_handle != 0 && "failed to parse config");
  CBERGAlefHandle engine = cberg_create_engine(config_handle);
  cberg_crawl_config_free(config_handle);
  assert(engine != 0 && "failed to create engine");
...
void test_scrape_asset_type_filter(void) {
  ...
  if (mock_per_fixture && mock_per_fixture[0] != '\0') {
    ...
    assert(mock_base != NULL && "MOCK_SERVER_URL must be set");
    snprintf(url, sizeof(url), "%s/fixtures/scrape_asset_type_filter",
             mock_base);
  }
  CBERGAlefHandle result = cberg_scrape(engine, url);
  assert(result != 0 && "expected call to succeed");
  ...
  assert(assets_length == 1 && "equals assertion failed");
  {
    ...
    for (int _wc_i = 0; _wc_i < _wc_count; _wc_i++) {
      ...
      if (_wc_val != NULL && strstr(_wc_val, "image") != NULL) {
        found = 1;
      }
      free(_wc_val);
      free(_wc_elem);
...
void test_scrape_basic_html_page(void) {
  ...
  CBERGAlefHandle config_handle = cberg_crawl_config_from_json(
      "{\"max_depth\":0,\"respect_robots_txt\":false}");
  assert(config_handle != 0 && "failed to parse config");
  CBERGAlefHandle engine = cberg_create_engine(config_handle);
  cberg_crawl_config_free(config_handle);
  assert(engine != 0 && "failed to create engine");
  ...
  assert(status_code == 200 && "equals assertion failed");
  assert(content_type != NULL && strcmp(content_type, "text/html") == 0 &&
         "equals assertion failed");
  assert(html != NULL && strlen(html) > 0 && strcmp(html, "[]") != 0 &&
         strcmp(html, "{}") != 0 && "expected non-empty value");
  assert(metadata_title != NULL &&
         strcmp(metadata_title, "Example Domain") == 0 &&
         "equals assertion failed");
  assert(metadata_description != NULL &&
         strstr(metadata_description, "illustrative examples") != NULL &&
  ...
  assert(links_length > 0 && "expected greater than");
  {
    ...
    for (int _wc_i = 0; _wc_i < _wc_count; _wc_i++) {
      ...
      if (_wc_val != NULL && strstr(_wc_val, "external") != NULL) {
        found = 1;
      }
      free(_wc_val);
      free(_wc_elem);
  ...
  assert((og_title == NULL || strlen(og_title) == 0 ||
          strcmp(og_title, "[]") == 0 || strcmp(og_title, "{}") == 0) &&
...
void test_scrape_dublin_core(void) {
  ...
  assert(status_code == 200 && "equals assertion failed");
  assert(dublin_core_title != NULL && strlen(dublin_core_title) > 0 &&
         strcmp(dublin_core_title, "[]") != 0 &&
         strcmp(dublin_core_title, "{}") != 0 && "expected non-empty value");
  assert(dublin_core_title != NULL &&
         strcmp(dublin_core_title,
                "Effects of Climate Change on Marine Biodiversity") == 0 &&
         "equals assertion failed");
  assert(dublin_core_creator != NULL &&
         strcmp(dublin_core_creator, "Dr. Jane Smith") == 0 &&
         "equals assertion failed");
...
void test_scrape_image_sources(void) {
  ...
  assert(og_image != NULL &&
         strcmp(og_image, "https://example.com/images/og-hero.jpg") == 0 &&
         "equals assertion failed");
...
void test_scrape_json_ld(void) {
  ...
  assert(status_code == 200 && "equals assertion failed");
  assert(json_ld != NULL && strlen(json_ld) > 0 && strcmp(json_ld, "[]") != 0 &&
         strcmp(json_ld, "{}") != 0 && "expected non-empty value");
  {
    int found = 0;
    ...
    for (int _wc_i = 0; _wc_i < _wc_count; _wc_i++) {
      ...
      if (_wc_val != NULL && strcmp(_wc_val, "Recipe") == 0) {
        found = 1;
      }
      free(_wc_val);
      free(_wc_elem);
  ...
  }
  {
    ...
    for (int _wc_i = 0; _wc_i < _wc_count; _wc_i++) {
      ...
      if (_wc_val != NULL && strcmp(_wc_val, "Best Chocolate Cake") == 0) {
        found = 1;
      }
      free(_wc_val);
      free(_wc_elem);
...
void test_scrape_malformed_html(void) {
  ...
  assert(html != NULL && strlen(html) > 0 && strcmp(html, "[]") != 0 &&
         strcmp(html, "{}") != 0 && "expected non-empty value");
  assert(metadata_description != NULL &&
         strstr(metadata_description, "broken HTML") != NULL &&
         "expected to contain substring");
  cberg_free_string(html);
  cberg_free_string(metadata_description);
  cberg_page_metadata_free(metadata_handle);
...
void test_scrape_og_metadata(void) {
  ...
  assert(og_title != NULL && strlen(og_title) > 0 &&
         strcmp(og_title, "[]") != 0 && strcmp(og_title, "{}") != 0 &&
         "expected non-empty value");
  assert(og_title != NULL && strcmp(og_title, "Article Title") == 0 &&
         "equals assertion failed");
  assert(og_type != NULL && strcmp(og_type, "article") == 0 &&
         "equals assertion failed");
  assert(og_image != NULL &&
         strcmp(og_image, "https://example.com/images/article-hero.jpg") == 0 &&
         "equals assertion failed");
  assert(og_description != NULL && strlen(og_description) > 0 &&
         strcmp(og_description, "[]") != 0 &&
         strcmp(og_description, "{}") != 0 && "expected non-empty value");
  assert(metadata_title != NULL &&
         strcmp(metadata_title, "Article Title - Example Blog") == 0 &&
         "equals assertion failed");
  cberg_free_string(og_title);
...
void test_scrape_twitter_card(void) {
  ...
  assert(status_code == 200 && "equals assertion failed");
  assert(twitter_card != NULL && strlen(twitter_card) > 0 &&
         strcmp(twitter_card, "[]") != 0 && strcmp(twitter_card, "{}") != 0 &&
         "expected non-empty value");
  assert(twitter_card_type != NULL &&
         strcmp(twitter_card_type, "summary_large_image") == 0 &&
         "equals assertion failed");
  assert(twitter_title != NULL &&
         strcmp(twitter_title, "New Product Launch") == 0 &&
         "equals assertion failed");
