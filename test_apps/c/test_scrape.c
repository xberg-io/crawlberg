...
void test_scrape_basic_html_page(void) {
  ...
  assert(status_code == 200 && "equals assertion failed");
  assert(str_trim_eq(content_type, "text/html") == 0 &&
         "equals assertion failed");
  assert(html != NULL && strlen(html) > 0 && "expected non-empty value");
  assert(str_trim_eq(metadata_title, "Example Domain") == 0 &&
         "equals assertion failed");
...
void test_scrape_dublin_core(void) {
  ...
  assert(dublin_core_title != NULL && strlen(dublin_core_title) > 0 &&
         "expected non-empty value");
  assert(str_trim_eq(dublin_core_title,
                     "Effects of Climate Change on Marine Biodiversity") == 0 &&
         "equals assertion failed");
  assert(str_trim_eq(dublin_core_creator, "Dr. Jane Smith") == 0 &&
         "equals assertion failed");
...
void test_scrape_image_sources(void) {
  ...
  assert(str_trim_eq(og_image, "https://example.com/images/og-hero.jpg") == 0 &&
         "equals assertion failed");
...
void test_scrape_json_ld(void) {
  ...
  assert(str_trim_eq(json_ld_name, "Best Chocolate Cake") == 0 &&
         "equals assertion failed");
...
void test_scrape_og_metadata(void) {
  ...
  assert(og_title != NULL && strlen(og_title) > 0 &&
         "expected non-empty value");
  assert(str_trim_eq(og_title, "Article Title") == 0 &&
         "equals assertion failed");
  assert(str_trim_eq(og_type, "article") == 0 && "equals assertion failed");
  assert(str_trim_eq(og_image, "https://example.com/images/article-hero.jpg") ==
             0 &&
         "equals assertion failed");
  assert(og_description != NULL && strlen(og_description) > 0 &&
         "expected non-empty value");
  assert(str_trim_eq(metadata_title, "Article Title - Example Blog") == 0 &&
         "equals assertion failed");
  cberg_free_string(og_title);
...
void test_scrape_twitter_card(void) {
  ...
  assert(twitter_card != NULL && strlen(twitter_card) > 0 &&
         "expected non-empty value");
  assert(str_trim_eq(twitter_card_type, "summary_large_image") == 0 &&
         "equals assertion failed");
  assert(str_trim_eq(twitter_title, "New Product Launch") == 0 &&
         "equals assertion failed");
