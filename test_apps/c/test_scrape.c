 ...
 * alef:hash:ac137370b46d1da091126ca0af623affbe54938958be522f8d2aca220091d8cd
...
void test_scrape_asset_dedup(void) {
  ...
  char *assets__content_hash =
      alef_json_get_string(assets_json, "content_hash");
  assert(status_code == 200 && "equals assertion failed");
  assert(assets_length == 2 && "equals assertion failed");
  assert(assets__content_hash != NULL && strlen(assets__content_hash) > 0 &&
...
void test_scrape_asset_type_filter(void) {
  ...
  assert(assets__category != NULL &&
         strstr(assets__category, "image") != NULL &&
...
void test_scrape_basic_html_page(void) {
  ...
  assert(links__link_type != NULL &&
         strstr(links__link_type, "external") != NULL &&
         "expected to contain substring");
...
void test_scrape_complex_links(void) {
  ...
  assert(links__url != NULL && strlen(links__url) > 0 &&
         "expected non-empty value");
  free(links__url);
  cberg_free_string(links_json);
  cberg_scrape_result_free(result);
...
void test_scrape_json_ld(void) {
  ...
  assert(json_ld_type != NULL && strcmp(json_ld_type, "Recipe") == 0 &&
         "equals assertion failed");
  assert(json_ld_name != NULL &&
         strcmp(json_ld_name, "Best Chocolate Cake") == 0 &&
         "equals assertion failed");
