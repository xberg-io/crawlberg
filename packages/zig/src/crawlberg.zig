...
inline fn _error_with_message(comptime E: type) E {
    ...
    if (E == CrawlError) return _from_ffi_msg_CrawlError(msg_opt);
    if (E == SsrfError) return _from_ffi_msg_SsrfError(msg_opt);
...
/// Map an FFI error message string to a `CrawlError` variant by prefix-matching
/// against the upstream `#[error("...")]` templates. Falls back to the
/// first declared variant when no prefix matches.
inline fn _from_ffi_msg_CrawlError(msg_opt: ?[]const u8) CrawlError {
    if (msg_opt) |msg| {
        if (std.mem.startsWith(u8, msg, "not_found:")) return error.NotFound;
        if (std.mem.startsWith(u8, msg, "unauthorized:")) return error.Unauthorized;
        if (std.mem.startsWith(u8, msg, "forbidden:")) return error.Forbidden;
        if (std.mem.startsWith(u8, msg, "forbidden: waf/blocked:")) return error.WafBlocked;
        if (std.mem.startsWith(u8, msg, "timeout:")) return error.Timeout;
        if (std.mem.startsWith(u8, msg, "rate_limited:")) return error.RateLimited;
        if (std.mem.startsWith(u8, msg, "server_error:")) return error.ServerError;
        if (std.mem.startsWith(u8, msg, "bad_gateway:")) return error.BadGateway;
        if (std.mem.startsWith(u8, msg, "gone:")) return error.Gone;
        if (std.mem.startsWith(u8, msg, "connection:")) return error.Connection;
        if (std.mem.startsWith(u8, msg, "dns:")) return error.Dns;
        if (std.mem.startsWith(u8, msg, "ssl:")) return error.Ssl;
        if (std.mem.startsWith(u8, msg, "data_loss:")) return error.DataLoss;
        if (std.mem.startsWith(u8, msg, "browser:")) return error.BrowserError;
        if (std.mem.startsWith(u8, msg, "browser_timeout:")) return error.BrowserTimeout;
        if (std.mem.startsWith(u8, msg, "invalid_config:")) return error.InvalidConfig;
        if (std.mem.startsWith(u8, msg, "unsupported:")) return error.Unsupported;
        if (std.mem.startsWith(u8, msg, "ssrf_policy_violation:")) return error.SsrfPolicyViolation;
        if (std.mem.startsWith(u8, msg, "other:")) return error.Other;
    }
...
/// Map an FFI error message string to a `SsrfError` variant by prefix-matching
/// against the upstream `#[error("...")]` templates. Falls back to the
/// first declared variant when no prefix matches.
inline fn _from_ffi_msg_SsrfError(msg_opt: ?[]const u8) SsrfError {
    if (msg_opt) |msg| {
        if (std.mem.startsWith(u8, msg, "denied by SSRF policy:")) return error.DeniedByPolicy;
        if (std.mem.startsWith(u8, msg, "host not on allowlist")) return error.NotOnAllowlist;
        if (std.mem.startsWith(u8, msg, "invalid CIDR in SSRF allowlist:")) return error.InvalidCidr;
        if (std.mem.startsWith(u8, msg, "dns resolution failed:")) return error.DnsResolutionFailed;
        if (std.mem.startsWith(u8, msg, "invalid URL:")) return error.InvalidUrl;
        if (std.mem.startsWith(u8, msg, "disallowed scheme:")) return error.DisallowedScheme;
        if (std.mem.startsWith(u8, msg, "too many redirects")) return error.TooManyRedirects;
    }
    return _first_error(SsrfError);
}
...
pub fn generate_citations(markdown: []const u8) error{OutOfMemory}![]u8 {
    ...
    return blk: {
        const _json_ptr = c.cberg_citation_result_to_json(_result.?);
        c.cberg_citation_result_free(_result.?);
...
pub fn create_engine(config: ?[]const u8) CrawlError!CrawlEngineHandle {
    ...
    const config_handle = if (config_z) |z| c.cberg_crawl_config_from_json(z) else null;
    if (config_z != null and config_handle == null) return _error_with_message(CrawlError);
    defer if (config_handle) |h| c.cberg_crawl_config_free(h);
...
pub fn scrape(engine: ?[]const u8, url: []const u8) CrawlError![]u8 {
    const engine_config_z: ?[:0]u8 = if (engine) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    const engine_config_handle = if (engine_config_z) |z| c.cberg_crawl_config_from_json(z) else null;
    if (engine_config_z != null and engine_config_handle == null) return _error_with_message(CrawlError);
    defer if (engine_config_handle) |h| c.cberg_crawl_config_free(h);
    const engine_handle = c.cberg_create_engine(engine_config_handle);
    if (engine_handle == null) return _error_with_message(CrawlError);
    ...
    return blk: {
        const _json_ptr = c.cberg_scrape_result_to_json(_result.?);
        c.cberg_scrape_result_free(_result.?);
        if (_json_ptr == null) return _first_error(CrawlError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
        break :blk owned;
...
pub fn crawl(engine: ?[]const u8, url: []const u8) CrawlError![]u8 {
    ...
    if (engine_config_z) |z| std.heap.c_allocator.free(z);
    return blk: {
        const _json_ptr = c.cberg_crawl_result_to_json(_result.?);
        c.cberg_crawl_result_free(_result.?);
        if (_json_ptr == null) return _first_error(CrawlError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn map_urls(engine: ?[]const u8, url: []const u8) CrawlError![]u8 {
    ...
    if (engine_config_z) |z| std.heap.c_allocator.free(z);
    return blk: {
        const _json_ptr = c.cberg_map_result_to_json(_result.?);
        c.cberg_map_result_free(_result.?);
        if (_json_ptr == null) return _first_error(CrawlError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn interact(engine: ?[]const u8, url: []const u8, actions: []const u8) CrawlError![]u8 {
    ...
    if (engine_config_z) |z| std.heap.c_allocator.free(z);
    return blk: {
        const _json_ptr = c.cberg_interaction_result_to_json(_result.?);
        c.cberg_interaction_result_free(_result.?);
        if (_json_ptr == null) return _first_error(CrawlError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn batch_scrape(engine: ?[]const u8, urls: []const u8) CrawlError![]u8 {
    ...
    if (engine_config_z) |z| std.heap.c_allocator.free(z);
    return blk: {
        const _json_ptr = c.cberg_batch_scrape_results_to_json(_result.?);
        c.cberg_batch_scrape_results_free(_result.?);
        if (_json_ptr == null) return _first_error(CrawlError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn batch_crawl(engine: ?[]const u8, urls: []const u8) CrawlError![]u8 {
    ...
    if (engine_config_z) |z| std.heap.c_allocator.free(z);
    return blk: {
        const _json_ptr = c.cberg_batch_crawl_results_to_json(_result.?);
        c.cberg_batch_crawl_results_free(_result.?);
        if (_json_ptr == null) return _first_error(CrawlError); // FFI to_json returned NULL — surface first declared variant rather than crashing on null-slice
        defer _free_string(_json_ptr);
        const slice = std.mem.sliceTo(_json_ptr, 0);
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub const CrawlEventStream = struct {
    _handle: ?*c.CBERGCrawlEventStream,

    /// Fetch the next item from the stream, or null at end-of-stream.
    /// Returns an error on mid-stream failure; null on clean EOS.
    pub fn next(self: *CrawlEventStream) (CrawlError||error{OutOfMemory,HandleClosed})!?CrawlEvent {
    const handle = self._handle orelse return error.HandleClosed;
        ...
        if (_chunk == null) {
            ...
            if (c.cberg_last_error_code() != 0) return _first_error(CrawlError);
            return null;
        }
...
pub const CrawlEngineHandle = struct {
    ...
    pub fn crawl_stream(self: *CrawlEngineHandle, req: []const u8) (CrawlError||error{OutOfMemory,HandleClosed})!CrawlEventStream {
    const handle = self._handle orelse return error.HandleClosed;
        ...
        const _stream_handle = c.cberg_crawl_engine_handle_crawl_stream_start(@as(*c.CBERGCrawlEngineHandle, @ptrCast(handle)), req_handle);
    ...
    /// Language bindings expose this as the native streaming shape for each
    /// target. WASM does not expose native streaming wrappers.
    pub fn batch_crawl_stream(self: *CrawlEngineHandle, req: []const u8) (CrawlError||error{OutOfMemory,HandleClosed})!CrawlEventStream {
    const handle = self._handle orelse return error.HandleClosed;
        const req_z = try std.heap.c_allocator.dupeZ(u8, req);
        ...
        const _stream_handle = c.cberg_crawl_engine_handle_batch_crawl_stream_start(@as(*c.CBERGCrawlEngineHandle, @ptrCast(handle)), req_handle);
        if (_stream_handle == null) { return _first_error(CrawlError); }
        return CrawlEventStream{ ._handle = _stream_handle };
    }
    ...
    const handle = self._handle orelse return;
        self._handle = null;
        c.cberg_crawl_engine_handle_free(@as(*c.CBERGCrawlEngineHandle, @ptrCast(handle)));
    }
};
