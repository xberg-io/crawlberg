...
// alef:hash:42684c5526d6c52de136f6ce05050bc1834f8f5e3013985eee59db5b55ae25f1
@file:Suppress(
...
object Crawlberg {
    ...
    fun batchCrawl(engine: CrawlEngineHandle, urls: List<String>): BatchCrawlResults {
        val resultJson = CrawlbergBridge.nativeBatchCrawl(engine.handle, mapper.writeValueAsString(urls))
    ...
    fun batchScrape(engine: CrawlEngineHandle, urls: List<String>): BatchScrapeResults {
        val resultJson = CrawlbergBridge.nativeBatchScrape(engine.handle, mapper.writeValueAsString(urls))
    ...
    fun crawl(engine: CrawlEngineHandle, url: String): CrawlResult {
        val resultJson = CrawlbergBridge.nativeCrawl(engine.handle, url)
     ...
     */
    fun createEngine(config: CrawlConfig? = null): CrawlEngineHandle = CrawlEngineHandle(CrawlbergBridge.nativeCreateEngine(config?.let { mapper.writeValueAsString(it) } ?: ""))
    /**
    ...
    fun generateCitations(markdown: String): CitationResult {
        val resultJson = CrawlbergBridge.nativeGenerateCitations(markdown)
    ...
    fun interact(engine: CrawlEngineHandle, url: String, actions: List<PageAction>): InteractionResult {
        val resultJson = CrawlbergBridge.nativeInteract(engine.handle, url, mapper.writerFor(mapper.typeFactory.constructCollectionType(List::class.java, PageAction::class.java)).writeValueAsString(actions))
    ...
    fun mapUrls(engine: CrawlEngineHandle, url: String): MapResult {
        val resultJson = CrawlbergBridge.nativeMapUrls(engine.handle, url)
    ...
    fun scrape(engine: CrawlEngineHandle, url: String): ScrapeResult {
        val resultJson = CrawlbergBridge.nativeScrape(engine.handle, url)
