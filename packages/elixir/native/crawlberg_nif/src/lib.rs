...
// alef:hash:cdf3e9172c88d5e26684c1c53fb449e2c53e0a163b521b7487419455ec0b506d
    ...
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            session_affinity: opts
                ...
                .and_then(|t| t.decode().ok())
                .unwrap_or(true),
            chrome_path: opts.get("chrome_path").and_then(|t| t.decode().ok()),
            chrome_args: opts
                .get("chrome_args")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
    ...
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            exclude_paths: opts
                ...
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
            path_patterns_match_query: opts
                .get("path_patterns_match_query")
                .and_then(|t| t.decode().ok())
                .unwrap_or(false),
            dedup_include_query: opts
                .get("dedup_include_query")
                .and_then(|t| t.decode().ok())
                .unwrap_or(false),
            strip_tracking_params: opts
                .get("strip_tracking_params")
                .and_then(|t| t.decode().ok())
                .unwrap_or(false),
            tracking_params: opts
                .get("tracking_params")
                .and_then(|t| t.decode().ok())
                .unwrap_or(vec![
                    "utm_*".to_string(),
            ...
            retry_codes: opts
                ...
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
            retry_initial_delay_ms: opts
                .get("retry_initial_delay_ms")
                .and_then(|t| t.decode().ok())
                .unwrap_or(100),
            retry_max_delay_ms: opts
                .get("retry_max_delay_ms")
                .and_then(|t| t.decode().ok())
                .unwrap_or(60000),
            rate_limit_jitter_ratio: opts
                .get("rate_limit_jitter_ratio")
                .and_then(|t| t.decode().ok())
                .unwrap_or(0.0),
...
pub struct CrawlEngineHandle {
    inner: Arc<std::sync::RwLock<crawlberg::CrawlEngineHandle>>,
}
...
impl DownloadedDocument {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            truncated: opts.get("truncated").and_then(|t| t.decode().ok()).unwrap_or_default(),
            content_path: opts.get("content_path").and_then(|t| t.decode().ok()),
            content_base64: opts.get("content_base64").and_then(|t| t.decode().ok()),
        }
    }
}
...
impl InteractionResult {
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            final_url: opts.get("final_url").and_then(|t| t.decode().ok()).unwrap_or_default(),
            screenshot_base64: opts.get("screenshot_base64").and_then(|t| t.decode().ok()),
        }
    }
}
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        crawlberg::batch_crawl(&engine.inner.read().unwrap_or_else(|e| e.into_inner()).clone(), urls)
                            .await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result.into())
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        crawlberg::batch_scrape(&engine.inner.read().unwrap_or_else(|e| e.into_inner()).clone(), urls)
                            .await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result.into())
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        crawlberg::crawl(&engine.inner.read().unwrap_or_else(|e| e.into_inner()).clone(), &url).await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result.into())
            })
...
pub fn create_engine(config: Option<String>) -> Result<ResourceArc<CrawlEngineHandle>, String> {
    let config_core: Option<crawlberg::CrawlConfig> = config
        ...
        .transpose()
        .map_err(|error| format!("failed to deserialize parameter `config` for `create_engine`: {error}"))?;
...
pub fn interact_async(
    ...
    let actions_core_option: Option<Vec<crawlberg::PageAction>> = actions
        .map(|s| serde_json::from_str::<Vec<crawlberg::PageAction>>(&s))
        .transpose()
        .map_err(|error| format!("failed to deserialize parameter `actions` for `interact_async`: {error}"))?;
    let actions_core: Vec<crawlberg::PageAction> = actions_core_option.unwrap_or_default();
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            .stack_size(32 * 1024 * 1024)
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        crawlberg::interact(
                            &engine.inner.read().unwrap_or_else(|e| e.into_inner()).clone(),
                            &url,
                            actions_core,
                        )
                        .await
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        crawlberg::map_urls(&engine.inner.read().unwrap_or_else(|e| e.into_inner()).clone(), &url).await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result.into())
            })
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                let result = rt
                    .block_on(async {
                        crawlberg::scrape(&engine.inner.read().unwrap_or_else(|e| e.into_inner()).clone(), &url).await
                    })
                    .map_err(|e| e.to_string())?;
                Ok(result.into())
            })
...
#[rustler::nif]
pub fn crawlconfig_validate(obj: String) -> Result<(), String> {
    let obj: crawlberg::CrawlConfig = serde_json::from_str::<crawlberg::CrawlConfig>(&obj)
        .map_err(|error| format!("failed to deserialize parameter `obj` for `crawlconfig_validate`: {error}"))?;
