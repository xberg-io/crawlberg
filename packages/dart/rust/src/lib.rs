...
// alef:hash:5c6f193dcb7b4f50f8a7a2b5b31700ead3941a4b4c5bbbcd585825a50d7c9c2b
...
impl From<crawlberg::CrawlConfig> for CrawlConfig {
    fn from(v: crawlberg::CrawlConfig) -> Self {
        CrawlConfig {
            ...
            path_patterns_match_query: v.path_patterns_match_query as _,
            dedup_include_query: v.dedup_include_query as _,
            strip_tracking_params: v.strip_tracking_params as _,
            tracking_params: v.tracking_params.into_iter().map(|s| s.into()).collect::<Vec<_>>(),
            custom_headers: v
                .custom_headers
                .into_iter()
            ...
            retry_initial_delay_ms: v.retry_initial_delay_ms as _,
            retry_max_delay_ms: v.retry_max_delay_ms as _,
            rate_limit_jitter_ratio: v.rate_limit_jitter_ratio as _,
...
impl From<crawlberg::BrowserConfig> for BrowserConfig {
    fn from(v: crawlberg::BrowserConfig) -> Self {
        BrowserConfig {
            ...
            overall_timeout: v.overall_timeout.as_millis() as i64,
            shutdown_timeout: v.shutdown_timeout.as_millis() as i64,
            ...
            chrome_path: v.chrome_path.map(|p| p.to_string_lossy().into_owned()),
            chrome_args: v.chrome_args.into_iter().map(|s| s.into()).collect::<Vec<_>>(),
        }
    }
}
...
fn try_convert_link_type_from_core(value: crawlberg::LinkType) -> Result<LinkType, String> {
    let representable = match &value {
        crawlberg::LinkType::Internal => true,
        crawlberg::LinkType::External => true,
        crawlberg::LinkType::Anchor => true,
        crawlberg::LinkType::Document => true,
    };
    if !representable {
...
fn try_convert_image_source_from_core(value: crawlberg::ImageSource) -> Result<ImageSource, String> {
    let representable = match &value {
        crawlberg::ImageSource::Img => true,
        crawlberg::ImageSource::PictureSource => true,
        crawlberg::ImageSource::OgImage => true,
        crawlberg::ImageSource::TwitterImage => true,
    };
    if !representable {
        return Err("ImageSource contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ImageSource::from(value))
}
...
fn try_convert_feed_type_from_core(value: crawlberg::FeedType) -> Result<FeedType, String> {
    let representable = match &value {
        crawlberg::FeedType::Rss => true,
        crawlberg::FeedType::Atom => true,
        crawlberg::FeedType::JsonFeed => true,
    };
    if !representable {
        return Err("FeedType contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(FeedType::from(value))
}
...
impl From<crawlberg::CrawlEvent> for CrawlEvent {
    fn from(v: crawlberg::CrawlEvent) -> Self {
        match v {
            ...
            crawlberg::CrawlEvent::Error { url, error } => CrawlEvent::Error { url, error },
...
#[cfg(not(target_arch = "wasm32"))]
fn try_convert_crawl_event_from_core(value: crawlberg::CrawlEvent) -> Result<CrawlEvent, String> {
    let representable = match &value {
        crawlberg::CrawlEvent::Page { .. } => true,
        crawlberg::CrawlEvent::Error { .. } => true,
        crawlberg::CrawlEvent::Complete { .. } => true,
    };
    if !representable {
        return Err("CrawlEvent contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(CrawlEvent::from(value))
}

impl From<crawlberg::PageAction> for PageAction {
    fn from(v: crawlberg::PageAction) -> Self {
        match v {
            crawlberg::PageAction::Click { selector } => PageAction::Click { selector },
            crawlberg::PageAction::TypeText { selector, text } => PageAction::TypeText { selector, text },
            crawlberg::PageAction::Press { key } => PageAction::Press { key },
            ...
            crawlberg::PageAction::ExecuteJs { script } => PageAction::ExecuteJs { script },
            crawlberg::PageAction::Scrape => PageAction::Scrape,
        }
    }
}
fn try_convert_page_action_from_core(value: crawlberg::PageAction) -> Result<PageAction, String> {
    let representable = match &value {
        crawlberg::PageAction::Click { .. } => true,
        crawlberg::PageAction::TypeText { .. } => true,
        crawlberg::PageAction::Press { .. } => true,
        crawlberg::PageAction::Scroll { .. } => true,
        crawlberg::PageAction::Wait { .. } => true,
        crawlberg::PageAction::Screenshot { .. } => true,
        crawlberg::PageAction::ExecuteJs { .. } => true,
        crawlberg::PageAction::Scrape => true,
    };
    if !representable {
        return Err("PageAction contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(PageAction::from(value))
}
...
fn try_convert_scroll_direction_from_core(value: crawlberg::ScrollDirection) -> Result<ScrollDirection, String> {
    let representable = match &value {
        crawlberg::ScrollDirection::Up => true,
        crawlberg::ScrollDirection::Down => true,
    };
    if !representable {
        return Err("ScrollDirection contains a variant unavailable in the Dart binding".to_string());
    }
    Ok(ScrollDirection::from(value))
}
...
impl From<CrawlConfig> for crawlberg::CrawlConfig {
    fn from(v: CrawlConfig) -> Self {
        crawlberg::CrawlConfig {
            ...
            dedup_include_query: v.dedup_include_query as _,
            strip_tracking_params: v.strip_tracking_params as _,
            tracking_params: v.tracking_params.into_iter().map(Into::into).collect(),
            custom_headers: v
                .custom_headers
                .into_iter()
                .map(|(k, v)| (k.into(), v.into()))
...
impl From<BrowserConfig> for crawlberg::BrowserConfig {
    fn from(v: BrowserConfig) -> Self {
        crawlberg::BrowserConfig {
            ...
            overall_timeout: std::time::Duration::from_millis(v.overall_timeout as u64),
            shutdown_timeout: std::time::Duration::from_millis(v.shutdown_timeout as u64),
            wait: v.wait.into(),
            ...
            capture_network_events: v.capture_network_events as _,
            session_affinity: v.session_affinity as _,
            chrome_path: v.chrome_path.map(std::path::PathBuf::from),
            chrome_args: v.chrome_args.into_iter().map(Into::into).collect(),
            ..Default::default()
        }
    }
}
...
impl From<PageAction> for crawlberg::PageAction {
    fn from(v: PageAction) -> Self {
        match v {
            PageAction::Click { selector } => crawlberg::PageAction::Click { selector },
            PageAction::TypeText { selector, text } => crawlberg::PageAction::TypeText { selector, text },
            PageAction::Press { key } => crawlberg::PageAction::Press { key },
            PageAction::Scroll {
                direction,
                selector,
                amount,
            ...
            PageAction::ExecuteJs { script } => crawlberg::PageAction::ExecuteJs { script },
...
pub fn create_extraction_meta_from_json(json: String) -> Result<ExtractionMeta, String> {
    serde_json::from_str::<crawlberg::ExtractionMeta>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ExtractionMeta::from(value)))
}
...
pub fn create_browser_extras_from_json(json: String) -> Result<BrowserExtras, String> {
    serde_json::from_str::<crawlberg::BrowserExtras>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BrowserExtras::from(value)))
}
...
pub fn create_downloaded_document_from_json(json: String) -> Result<DownloadedDocument, String> {
    serde_json::from_str::<crawlberg::DownloadedDocument>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(DownloadedDocument::from(value)))
}
...
pub fn create_interaction_result_from_json(json: String) -> Result<InteractionResult, String> {
    serde_json::from_str::<crawlberg::InteractionResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(InteractionResult::from(value)))
}
...
pub fn create_action_result_from_json(json: String) -> Result<ActionResult, String> {
    serde_json::from_str::<crawlberg::ActionResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ActionResult::from(value)))
}
...
pub fn create_scrape_result_from_json(json: String) -> Result<ScrapeResult, String> {
    serde_json::from_str::<crawlberg::ScrapeResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ScrapeResult::from(value)))
}
...
pub fn create_crawl_page_result_from_json(json: String) -> Result<CrawlPageResult, String> {
    serde_json::from_str::<crawlberg::CrawlPageResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CrawlPageResult::from(value)))
}
...
pub fn create_crawl_result_from_json(json: String) -> Result<CrawlResult, String> {
    serde_json::from_str::<crawlberg::CrawlResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CrawlResult::from(value)))
}
...
pub fn create_markdown_result_from_json(json: String) -> Result<MarkdownResult, String> {
    serde_json::from_str::<crawlberg::MarkdownResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(MarkdownResult::from(value)))
}
...
pub fn create_link_info_from_json(json: String) -> Result<LinkInfo, String> {
    serde_json::from_str::<crawlberg::LinkInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(LinkInfo::from(value)))
}
...
pub fn create_image_info_from_json(json: String) -> Result<ImageInfo, String> {
    serde_json::from_str::<crawlberg::ImageInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ImageInfo::from(value)))
}
...
pub fn create_feed_info_from_json(json: String) -> Result<FeedInfo, String> {
    serde_json::from_str::<crawlberg::FeedInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FeedInfo::from(value)))
}
...
pub fn create_json_ld_entry_from_json(json: String) -> Result<JsonLdEntry, String> {
    serde_json::from_str::<crawlberg::JsonLdEntry>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(JsonLdEntry::from(value)))
}
...
pub fn create_cookie_info_from_json(json: String) -> Result<CookieInfo, String> {
    serde_json::from_str::<crawlberg::CookieInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CookieInfo::from(value)))
}
...
pub fn create_downloaded_asset_from_json(json: String) -> Result<DownloadedAsset, String> {
    serde_json::from_str::<crawlberg::DownloadedAsset>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(DownloadedAsset::from(value)))
}
...
pub fn create_article_metadata_from_json(json: String) -> Result<ArticleMetadata, String> {
    serde_json::from_str::<crawlberg::ArticleMetadata>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ArticleMetadata::from(value)))
}
...
pub fn create_hreflang_entry_from_json(json: String) -> Result<HreflangEntry, String> {
    serde_json::from_str::<crawlberg::HreflangEntry>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(HreflangEntry::from(value)))
}
...
pub fn create_favicon_info_from_json(json: String) -> Result<FaviconInfo, String> {
    serde_json::from_str::<crawlberg::FaviconInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(FaviconInfo::from(value)))
}
...
pub fn create_heading_info_from_json(json: String) -> Result<HeadingInfo, String> {
    serde_json::from_str::<crawlberg::HeadingInfo>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(HeadingInfo::from(value)))
}
...
pub fn create_response_meta_from_json(json: String) -> Result<ResponseMeta, String> {
    serde_json::from_str::<crawlberg::ResponseMeta>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(ResponseMeta::from(value)))
}
...
pub fn create_page_metadata_from_json(json: String) -> Result<PageMetadata, String> {
    serde_json::from_str::<crawlberg::PageMetadata>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(PageMetadata::from(value)))
}
...
pub fn create_crawl_stream_request_from_json(json: String) -> Result<CrawlStreamRequest, String> {
    serde_json::from_str::<crawlberg::CrawlStreamRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CrawlStreamRequest::from(value)))
}
...
pub fn create_batch_crawl_stream_request_from_json(json: String) -> Result<BatchCrawlStreamRequest, String> {
    serde_json::from_str::<crawlberg::BatchCrawlStreamRequest>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchCrawlStreamRequest::from(value)))
}
...
pub fn create_citation_result_from_json(json: String) -> Result<CitationResult, String> {
    serde_json::from_str::<crawlberg::CitationResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CitationResult::from(value)))
}
...
pub fn create_citation_reference_from_json(json: String) -> Result<CitationReference, String> {
    serde_json::from_str::<crawlberg::CitationReference>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(CitationReference::from(value)))
}
...
pub fn create_batch_scrape_result_from_json(json: String) -> Result<BatchScrapeResult, String> {
    serde_json::from_str::<crawlberg::BatchScrapeResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchScrapeResult::from(value)))
}
...
pub fn create_batch_crawl_result_from_json(json: String) -> Result<BatchCrawlResult, String> {
    serde_json::from_str::<crawlberg::BatchCrawlResult>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchCrawlResult::from(value)))
}
...
pub fn create_batch_scrape_results_from_json(json: String) -> Result<BatchScrapeResults, String> {
    serde_json::from_str::<crawlberg::BatchScrapeResults>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchScrapeResults::from(value)))
}
...
pub fn create_batch_crawl_results_from_json(json: String) -> Result<BatchCrawlResults, String> {
    serde_json::from_str::<crawlberg::BatchCrawlResults>(&json)
        .map_err(|e| e.to_string())
        .and_then(|value| Ok(BatchCrawlResults::from(value)))
}
...
#[frb]
pub fn create_image_source_from_json(json: String) -> Result<ImageSource, String> {
    serde_json::from_str::<crawlberg::ImageSource>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_image_source_from_core)
}

#[frb]
pub fn create_feed_type_from_json(json: String) -> Result<FeedType, String> {
    serde_json::from_str::<crawlberg::FeedType>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_feed_type_from_core)
}
...
#[cfg(not(target_arch = "wasm32"))]
#[frb]
pub fn create_crawl_event_from_json(json: String) -> Result<CrawlEvent, String> {
    serde_json::from_str::<crawlberg::CrawlEvent>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_crawl_event_from_core)
}

#[frb]
pub fn create_page_action_from_json(json: String) -> Result<PageAction, String> {
    serde_json::from_str::<crawlberg::PageAction>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_page_action_from_core)
}

#[frb]
pub fn create_scroll_direction_from_json(json: String) -> Result<ScrollDirection, String> {
    serde_json::from_str::<crawlberg::ScrollDirection>(&json)
        .map_err(|e| e.to_string())
        .and_then(try_convert_scroll_direction_from_core)
}
