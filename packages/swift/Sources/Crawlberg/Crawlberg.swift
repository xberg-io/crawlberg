...
// alef:hash:d88aec5aa4110ce09b7fc53f1b90389c3d003d1555f933088c2d8641ef003cfb
...
internal extension CitationResult {
    init(_ rb: RustBridge.CitationResultRef) throws {
        ...
        self.references = try rb.references().map { (s: RustStringRef) -> CitationReference in let d = s.as_str().toString().data(using: .utf8) ?? Data(); return try JSONDecoder().decode(CitationReference.self, from: d) }
    }
...
public struct ContentConfig: Codable, Sendable, Hashable {
    ...
    public init(outputFormat: String = "markdown", preprocessingPreset: String = "standard", removeNavigation: Bool = true, removeForms: Bool = true, stripTags: [String], preserveTags: [String], excludeSelectors: [String] = ["noscript"], skipImages: Bool = false, maxDepth: UInt? = nil, wrap: Bool = false, wrapWidth: UInt = 80, includeDocumentStructure: Bool = true, extractMetadata: Bool = true) {
...
internal extension ContentConfig {
    ...
    func intoRust() throws -> RustBridge.ContentConfig {
        ...
        return RustBridge.ContentConfig(RustString(self.outputFormat), RustString(self.preprocessingPreset), self.removeNavigation, self.removeForms, __stripTags, __preserveTags, __excludeSelectors, self.skipImages, self.maxDepth, self.wrap, self.wrapWidth, self.includeDocumentStructure, self.extractMetadata)
    }
}
...
public struct DownloadedDocument: Codable, Sendable, Hashable {
    ...
    public init(url: String, mimeType: String, size: UInt, filename: String? = nil, contentHash: String, headers: [String: String], truncated: Bool, contentPath: String? = nil, contentBase64: String? = nil) {
        self.url = url
        self.mimeType = mimeType
        self.size = size
    ...
    public init(from decoder: any Decoder) throws {
        ...
        self.contentPath = try container.decodeIfPresent(String.self, forKey: .contentPath) ?? nil
        self.contentBase64 = try container.decodeIfPresent(String.self, forKey: .contentBase64) ?? nil
    }
}

// MARK: - Internal FFI conversions for DownloadedDocument
internal extension DownloadedDocument {
    init(_ rb: RustBridge.DownloadedDocumentRef) throws {
        self.url = rb.url().toString()
        self.mimeType = rb.mimeType().toString()
        self.size = rb.size()
        self.filename = rb.filename()?.toString()
        self.contentHash = rb.contentHash().toString()
        self.headers = try JSONDecoder().decode([String: String].self, from: ((rb.headers().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.truncated = rb.truncated()
        self.contentPath = rb.contentPath()?.toString()
        self.contentBase64 = rb.contentBase64()?.toString()
    }
...
internal extension PageMetadata {
    init(_ rb: RustBridge.PageMetadataRef) throws {
        ...
        self.ogLocaleAlternates = try JSONDecoder().decode([String]?.self, from: ((rb.ogLocaleAlternates().toString()).data(using: .utf8) ?? Data("null".utf8)))
        ...
        self.hreflangs = try JSONDecoder().decode([HreflangEntry]?.self, from: ((rb.hreflangs().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.favicons = try JSONDecoder().decode([FaviconInfo]?.self, from: ((rb.favicons().toString()).data(using: .utf8) ?? Data("null".utf8)))
        self.headings = try JSONDecoder().decode([HeadingInfo]?.self, from: ((rb.headings().toString()).data(using: .utf8) ?? Data("null".utf8)))
...
public typealias ArticleMetadataRef = RustBridge.ArticleMetadataRef

public typealias ArticleMetadataRefMut = RustBridge.ArticleMetadataRefMut

public typealias BatchCrawlStreamRequestRef = RustBridge.BatchCrawlStreamRequestRef

public typealias BatchCrawlStreamRequestRefMut = RustBridge.BatchCrawlStreamRequestRefMut

public typealias CitationReferenceRef = RustBridge.CitationReferenceRef

public typealias CitationReferenceRefMut = RustBridge.CitationReferenceRefMut

public typealias CitationResultRef = RustBridge.CitationResultRef

public typealias CitationResultRefMut = RustBridge.CitationResultRefMut
...
public typealias CookieInfoRef = RustBridge.CookieInfoRef

public typealias CookieInfoRefMut = RustBridge.CookieInfoRefMut

public typealias CrawlStreamRequestRef = RustBridge.CrawlStreamRequestRef

public typealias CrawlStreamRequestRefMut = RustBridge.CrawlStreamRequestRefMut

public typealias DownloadedAssetRef = RustBridge.DownloadedAssetRef

public typealias DownloadedAssetRefMut = RustBridge.DownloadedAssetRefMut

public typealias DownloadedDocumentRef = RustBridge.DownloadedDocumentRef

public typealias DownloadedDocumentRefMut = RustBridge.DownloadedDocumentRefMut

public typealias ExtractionMetaRef = RustBridge.ExtractionMetaRef

public typealias ExtractionMetaRefMut = RustBridge.ExtractionMetaRefMut

public typealias FaviconInfoRef = RustBridge.FaviconInfoRef

public typealias FaviconInfoRefMut = RustBridge.FaviconInfoRefMut

public typealias FeedInfoRef = RustBridge.FeedInfoRef

public typealias FeedInfoRefMut = RustBridge.FeedInfoRefMut

public typealias HeadingInfoRef = RustBridge.HeadingInfoRef

public typealias HeadingInfoRefMut = RustBridge.HeadingInfoRefMut

public typealias HreflangEntryRef = RustBridge.HreflangEntryRef

public typealias HreflangEntryRefMut = RustBridge.HreflangEntryRefMut

public typealias ImageInfoRef = RustBridge.ImageInfoRef

public typealias ImageInfoRefMut = RustBridge.ImageInfoRefMut

public typealias JsonLdEntryRef = RustBridge.JsonLdEntryRef

public typealias JsonLdEntryRefMut = RustBridge.JsonLdEntryRefMut

public typealias LinkInfoRef = RustBridge.LinkInfoRef

public typealias LinkInfoRefMut = RustBridge.LinkInfoRefMut

public typealias MapResultRef = RustBridge.MapResultRef

public typealias MapResultRefMut = RustBridge.MapResultRefMut

public typealias PageMetadataRef = RustBridge.PageMetadataRef

public typealias PageMetadataRefMut = RustBridge.PageMetadataRefMut

public typealias ProxyConfigRef = RustBridge.ProxyConfigRef

public typealias ProxyConfigRefMut = RustBridge.ProxyConfigRefMut

public typealias ResponseMetaRef = RustBridge.ResponseMetaRef

public typealias ResponseMetaRefMut = RustBridge.ResponseMetaRefMut
...
public enum CrawlError: Swift.Error {
    ...
    case notFound(message: String, source: String?)
    /// The request was unauthorized (HTTP 401).
    case unauthorized(message: String, source: String?)
    /// The request was forbidden (HTTP 403).
    case forbidden(message: String, source: String?)
    ...
    case timeout(message: String, source: String?)
    /// The request was rate-limited (HTTP 429).
    case rateLimited(message: String, source: String?)
    /// A server error occurred (HTTP 5xx).
    case serverError(message: String, source: String?)
    /// A bad gateway error occurred (HTTP 502).
    case badGateway(message: String, source: String?)
    /// The resource is permanently gone (HTTP 410).
    case gone(message: String, source: String?)
    /// A connection error occurred.
    case connection(message: String, source: String?)
    /// A DNS resolution error occurred.
    case dns(message: String, source: String?)
    /// An SSL/TLS error occurred.
    case ssl(message: String, source: String?)
    /// Data was lost or truncated during transfer.
    case dataLoss(message: String, source: String?)
    /// The browser failed to launch, connect, or navigate.
    case browserError(message: String, source: String?)
    /// The browser page load or rendering timed out.
    case browserTimeout(message: String, source: String?)
    /// The provided configuration is invalid.
    case invalidConfig(message: String, source: String?)
    /// The requested capability is not supported by the active backend or build.
    case unsupported(message: String, source: String?)
    /// A URL was rejected by SSRF policy (private IP, metadata, disallowed scheme, etc).
    case ssrfPolicyViolation(message: String, url: String, reason: String, source: String?)
    /// An unclassified error occurred.
    case other(message: String, source: String?)
...
public enum SsrfError: Swift.Error {
    /// URL denied by SSRF policy: private IP, metadata IP, etc.
    case deniedByPolicy(message: String, reason: String)
    /// Host not on allowlist when an allowlist is configured.
    case notOnAllowlist
    /// Allowlist entry is not a parseable CIDR block.
    case invalidCidr(message: String, field0: String)
    /// DNS resolution failed for hostname.
    case dnsResolutionFailed(message: String, field0: String)
    /// Invalid URL format.
    case invalidUrl(message: String, field0: String)
    /// URL scheme not in allowlist (e.g., `ftp://` when only `http`/`https` allowed).
    case disallowedScheme(message: String, field0: String)
...
public func downloadedDocumentFromJson(_ json: String) throws -> DownloadedDocument {
    let data = json.data(using: .utf8) ?? Data()
    return try JSONDecoder().decode(DownloadedDocument.self, from: data)
}
