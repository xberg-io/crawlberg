...
public sealed record BrowserExtras {
  ...
  public List<ResponseMeta> NetworkEvents {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<BrowserExtras>(json, JsonOptions) ??
             throw new CrawlbergException(
                 $"Failed to parse BrowserExtras from JSON: deserializer returned null");
    } catch (CrawlbergException) {
      throw;
    } catch (Exception e) {
      throw new CrawlbergException(
          $"Failed to parse BrowserExtras from JSON: {e.Message}", e);
    }
  }
