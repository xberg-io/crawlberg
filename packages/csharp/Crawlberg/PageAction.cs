...
public abstract record PageAction {
  ...
  /// </summary>
  public sealed record Press([property : JsonPropertyName("key")] string Key)
      : PageAction;
...
public sealed class PageActionJsonConverter : JsonConverter<PageAction> {
  public override PageAction Read(ref Utf8JsonReader reader, Type typeToConvert,
    ...
    return tagValue switch {
      "click" =>
          JsonSerializer.Deserialize<PageAction.Click>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "type" =>
          JsonSerializer.Deserialize<PageAction.TypeText>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "press" =>
          JsonSerializer.Deserialize<PageAction.Press>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "scroll" =>
          JsonSerializer.Deserialize<PageAction.Scroll>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "wait" =>
          JsonSerializer.Deserialize<PageAction.Wait>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "screenshot" => JsonSerializer.Deserialize<PageAction.Screenshot>(
                          flatJson, options) ??
                      throw new JsonException("Failed to deserialize variant"),
      "executeJs" =>
          JsonSerializer.Deserialize<PageAction.ExecuteJs>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "scrape" => new PageAction.Scrape(), // Unknown discriminator: forward
                                           // compatibility for unknown variants
      _ => throw new JsonException(
          $"Unknown PageAction discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, PageAction value,
                             JsonSerializerOptions options) {
    ...
    default:
      throw new JsonException(
          $"Unknown PageAction variant: {value.GetType().Name}");
    }
