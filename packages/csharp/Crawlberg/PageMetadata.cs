...
/// <summary>
/// Metadata extracted from an HTML page's `&lt;meta&gt;` tags and
/// `&lt;title&gt;` element.
/// </summary>
    ...
    try {
      return JsonSerializer.Deserialize<PageMetadata>(json, JsonOptions) ??
             throw new CrawlbergException(
                 $"Failed to parse PageMetadata from JSON: deserializer returned null");
    } catch (CrawlbergException) {
      throw;
    } catch (Exception e) {
      throw new CrawlbergException(
          $"Failed to parse PageMetadata from JSON: {e.Message}", e);
    }
  }
