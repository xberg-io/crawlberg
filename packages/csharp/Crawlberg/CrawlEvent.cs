...
public sealed class CrawlEventJsonConverter : JsonConverter<CrawlEvent> {
  public override CrawlEvent Read(ref Utf8JsonReader reader, Type typeToConvert,
    ...
    return tagValue switch {
      "page" =>
          JsonSerializer.Deserialize<CrawlEvent.Page>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "error" =>
          JsonSerializer.Deserialize<CrawlEvent.Error>(flatJson, options) ??
          throw new JsonException("Failed to deserialize variant"),
      "complete" =>
          JsonSerializer.Deserialize<CrawlEvent.Complete>(flatJson, options) ??
          throw new JsonException(
              "Failed to deserialize variant"), // Unknown discriminator:
                                                // forward compatibility for
                                                // unknown variants
      _ => throw new JsonException(
          $"Unknown CrawlEvent discriminator: {tagValue}")
    };
  }

  public override void Write(Utf8JsonWriter writer, CrawlEvent value,
                             JsonSerializerOptions options) {
    ...
    default:
      throw new JsonException(
          $"Unknown CrawlEvent variant: {value.GetType().Name}");
    }
