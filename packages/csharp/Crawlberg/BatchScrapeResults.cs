...
/// <summary>
/// Aggregate result of a batch scrape, exposing per-URL results plus
/// precomputed counts.
///
...
public sealed record BatchScrapeResults {
  ...
  public static BatchScrapeResults FromJson(string json) {
    try {
      return JsonSerializer.Deserialize<BatchScrapeResults>(json,
                                                            JsonOptions) ??
             throw new CrawlbergException(
                 $"Failed to parse BatchScrapeResults from JSON: deserializer returned null");
    } catch (CrawlbergException) {
      throw;
    } catch (Exception e) {
      throw new CrawlbergException(
          $"Failed to parse BatchScrapeResults from JSON: {e.Message}", e);
    }
  }
