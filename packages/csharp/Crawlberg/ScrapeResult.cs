...
// alef:hash:d1bb04d9affb42e50dc42a1ca3df35fb65f8da27b128b5c17bff7f479a689833
...
public sealed record ScrapeResult {
  ...
  public List<JsonLdEntry> JsonLd {
    get; init;
  } = [];
  ...
  public List<DownloadedAsset> Assets {
    get; init;
  } = [];
    ...
    try {
      return JsonSerializer.Deserialize<ScrapeResult>(json, JsonOptions) ??
             throw new CrawlbergException(
                 $"Failed to parse ScrapeResult from JSON: deserializer returned null");
    } catch (CrawlbergException) {
      throw;
    } catch (Exception e) {
      throw new CrawlbergException(
          $"Failed to parse ScrapeResult from JSON: {e.Message}", e);
    }
  }
