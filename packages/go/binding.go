...
// alef:hash:e7b7fc63150dfd86e551636a55f61bf3e0720b623d1872d5472d15f054d9478a
...
// If linking fails with `cannot find -lcrawlberg_ffi`, run `go run <module>/cmd/setup`.

/*
#cgo CFLAGS: -I${SRCDIR}/include
#cgo CFLAGS: -DCBERG_FEATURE_BROWSER_CHROMIUMOXIDE=1 -DCBERG_FEATURE_INTERACT=1
...
func lastError() error {
	...
	var message string
	if ctx := C.cberg_last_error_context(); ctx != nil {
...
var (
	...
	// ErrInvalidCidr is returned when invalid CIDR in SSRF allowlist.
	ErrInvalidCidr = errors.New("invalid CIDR in SSRF allowlist")
...
// Sealed interface -- use one of CrawlEventPage, CrawlEventError, CrawlEventComplete.
...
// Sealed interface -- use one of PageActionClick, PageActionTypeText, PageActionPress, PageActionScroll, PageActionWait, PageActionScreenshot, PageActionExecuteJs, PageActionScrape.
...
type BrowserConfig struct {
	...
	// exit.
	OverallTimeout *uint64 `json:"overall_timeout,omitempty"`
...
type CrawlConfig struct {
	...
	PathPatternsMatchQuery bool `json:"path_patterns_match_query"`
	...
	PathPatternsMatchURL bool `json:"path_patterns_match_url"`
	...
	DedupIncludeQuery bool `json:"dedup_include_query"`
	...
	RetryInitialDelayMs *uint64 `json:"retry_initial_delay_ms,omitempty"`
	// Upper bound, in milliseconds, on the exponential retry backoff. Defaults to 60s.
	RetryMaxDelayMs *uint64 `json:"retry_max_delay_ms,omitempty"`
	...
	RateLimitJitterRatio float64 `json:"rate_limit_jitter_ratio"`
...
func (s *CrawlConfig) UnmarshalJSON(data []byte) error {
	...
	s.PathPatternsMatchQuery = raw.PathPatternsMatchQuery
	s.PathPatternsMatchURL = raw.PathPatternsMatchURL
	s.DedupIncludeQuery = raw.DedupIncludeQuery
	s.StripTrackingParams = raw.StripTrackingParams
	s.TrackingParams = raw.TrackingParams
	...
	s.RetryInitialDelayMs = raw.RetryInitialDelayMs
	s.RetryMaxDelayMs = raw.RetryMaxDelayMs
	s.RateLimitJitterRatio = raw.RateLimitJitterRatio
...
func CreateEngine(config *CrawlConfig) (*CrawlEngineHandle, error) {
	...
	return &CrawlEngineHandle{ptr: ptr}, nil
}
...
func Interact(engine *CrawlEngineHandle, url string, actions []PageAction) (*InteractionResult, error) {
	...
	// one the FFI's from_str accepts. Normalise so both spellings of "empty" cross the ABI
	// identically.
	if string(jsonBytescActions) == "null" {
		jsonBytescActions = []byte("[]")
	}
...
func (h *CrawlEngineHandle) CrawlStream(req CrawlStreamRequest) (<-chan CrawlEvent, error) {
	...
	handle := C.cberg_crawl_engine_handle_crawl_stream_start(h.ptr, cReq)
	if handle == 0 {
		if err := lastError(); err != nil {
			return nil, err
		}
...
func (h *CrawlEngineHandle) BatchCrawlStream(req BatchCrawlStreamRequest) (<-chan CrawlEvent, error) {
	...
	handle := C.cberg_crawl_engine_handle_batch_crawl_stream_start(h.ptr, cReq)
	if handle == 0 {
		if err := lastError(); err != nil {
			return nil, err
		}
