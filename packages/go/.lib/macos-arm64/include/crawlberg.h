 ...
 * ignored, `-1` reports an invalid handle (see `cberg_last_error_code`). #
 ...
 *
 * `cberg_crawl_config_max_links_per_page` cannot distinguish a `None` field
 ...
 * be ignored, `-1` reports an invalid handle (see `cberg_last_error_code`). #
 * Safety Pointer must be a valid handle returned by this library.
 */
 ...
 *
 * `cberg_crawl_config_document_url_depth` cannot distinguish a `None` field
 * from a legitimate zero-valued `Some` at the C ABI boundary -- there is no
 * null representation for a numeric return, so both collapse to the same
 * sentinel. Call this function first: `1` means the field getter's return value
 * is meaningful, `0` means the field is absent and the getter's sentinel must
 ...
 *
 * `cberg_crawl_config_document_max_size` cannot distinguish a `None` field from
 ...
 * `Some`.
 *
 * `cberg_crawl_config_ssrf_deny_private_explicit` cannot distinguish a `None`
 ...
 */
int32_t
cberg_crawl_config_has_ssrf_deny_private_explicit(CBERGAlefHandle handle);
 ...
 *
 * Only the first violation encountered is reported, not every one, so fixing a
 * rejected configuration can surface a further error on the next attempt.
 * \note SAFETY: Caller must ensure all pointer arguments are valid or null.
 * Returned pointers must be freed with the appropriate free function.
 */
 ...
 *
 * `cberg_extraction_meta_prompt_tokens` cannot distinguish a `None` field from
 * a legitimate zero-valued `Some` at the C ABI boundary -- there is no null
 * representation for a numeric return, so both collapse to the same sentinel.
 * Call this function first: `1` means the field getter's return value is
 * meaningful, `0` means the field is absent and the getter's sentinel must be
 ...
 *
 * `cberg_extraction_meta_completion_tokens` cannot distinguish a `None` field
 * from a legitimate zero-valued `Some` at the C ABI boundary -- there is no
 * null representation for a numeric return, so both collapse to the same
 * sentinel. Call this function first: `1` means the field getter's return value
 * is meaningful, `0` means the field is absent and the getter's sentinel must
